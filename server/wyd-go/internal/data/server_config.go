...
type ServerConfig struct {
	...
	// DatabasePath e o arquivo do banco SQLite (database_driver=sqlite).
	DatabasePath                 string
	DatabaseURL                  string
	...
	DebugAddress string
	// AdminUser e AdminPassword sao as credenciais do painel de
	// administracao (janela --gui, console --cli e painel TCP). O painel usa
	// senha unica de admin (nao ha multiusuarios); AdminUser existe para
	// exibicao/auditoria. Prioridade: flag/env WYD_ADMIN_PASSWORD > server.txt
	// > aleatoria logada no boot. Loopback por padrao; para acesso remoto,
	// tunel SSH.
	AdminUser     string
	...
	// ClientServerListPath e o serverlist.bin do CLIENT (pasta do jogo) que os
	// comandos serverlist.* do painel geram/atualizam. Vazio desliga a feature
	// (a aba Lista da GUI avisa como configurar). Caminhos relativos seguem o
	// diretorio de trabalho do servidor.
	ClientServerListPath string
...
func DefaultServerConfig() ServerConfig {
	return ServerConfig{
		...
		AdminUser:                    "admin",
		AdminPassword:                "", // vazio = aleatoria logada no boot (ou env/flag)
		LoadtestAccountPrefix:        "",
		ClientServerListPath:         "", // vazio = feature serverlist desligada
...
func LoadServerConfig(path string) (ServerConfig, error) {
	...
	setters := map[string]func(string) error{
		...
		"database_path":                    func(v string) error { cfg.DatabasePath = v; return nil },
		...
		"admin_user":                       func(v string) error { cfg.AdminUser = strings.TrimSpace(v); return nil },
		"admin_password":                   func(v string) error { cfg.AdminPassword = strings.TrimSpace(v); return nil },
		...
		"client_serverlist": func(v string) error { cfg.ClientServerListPath = strings.TrimSpace(v); return nil },
	...
	case "postgres", "sqlite", "json":
	default:
		return ServerConfig{}, fmt.Errorf("%s: database_driver invalido %q (use postgres, sqlite ou json)",

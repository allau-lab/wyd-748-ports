...
func (s *JSONStore) prepareAccountItemUIDs(accounts ...*model.Account) (
	...
	return prepareItemUIDsInto(s.itemOwners, accounts...)
}

// prepareItemUIDsInto e o core compartilhado entre as stores (JSON e SQLite):
// recebe o indice atual e devolve o proximo indice com os UIDs das contas do
// lote canonicalizados/gerados, sem publicar nada no store.
func prepareItemUIDsInto(owners map[string]itemUIDOwner, accounts ...*model.Account) (
	map[string]itemUIDOwner, int, error,
) {
	if _, err := prepareAccountCharacterUIDs(accounts...); err != nil {
		return nil, 0, err
	}
	next := cloneItemOwners(owners)

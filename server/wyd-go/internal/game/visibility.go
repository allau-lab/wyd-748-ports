...
func (w *World) showMob(p *Player, m *Mob) {
	...
	p.Session.Send(wire.CreateMobVisualFor(p.ClientABI, m.ID, m.Def.Name, m.X, m.Y,
...
func playerEnterViewPackets(observer, subject *Player) [][]byte {
	if subject == nil || subject.Char == nil {
		return nil
	}
	abi := wire.ABI748
	if observer != nil {
		abi = observer.ClientABI
	}
	return [][]byte{
		wire.CreateMobWithGuildRankFor(abi, subject.ID, subject.Char.Name, subject.X, subject.Y,
		...
		wire.HpMpFor(abi, subject.ID, wireScoreState(subject.Char)),
...
func sendPlayerEnterView(observer, subject *Player) {
	...
	for _, pkt := range playerEnterViewPackets(observer, subject) {
...
func (w *World) publishPlayerAffects(subject *Player) {
	...
	if affects := playerAffectsPacket(subject); len(affects) > 0 {
		subject.Session.Send(affects)
	}
}
...
func (w *World) syncPlayerVitals(subject *Player) {
	...
	w.sendToPlayerViewProtocol(subject, func(observer *Player) []byte {
		return wire.HpMpFor(observer.ClientABI, subject.ID, wireScoreState(subject.Char))
	})
}
...
func (w *World) syncPlayerVitalsToObservers(subject *Player) {
	...
	for _, p := range w.nearbyWorldPlayers(subject.X, subject.Y, viewHalfX) {
		if p != subject && w.playersVisibleTogether(p, subject) && p.hasVisible(subject.ID) {
			p.Session.Send(wire.HpMpFor(p.ClientABI, subject.ID, wireScoreState(subject.Char)))
		}
	}
}
...
func (w *World) syncPlayerChaos(subject *Player) {
	...
	w.sendToPlayerViewProtocol(subject, func(observer *Player) []byte {
		return wire.CreateMobWithGuildRankFor(observer.ClientABI, subject.ID, subject.Char.Name, subject.X, subject.Y,
			bodyMesh(subject.Char), bodyAncient(subject.Char), wireScoreState(subject.Char),
			subject.Char.Affects[:], 2, subject.Char.GuildID, subject.Char.GuildRank, subject.Char.CP)
	})
}

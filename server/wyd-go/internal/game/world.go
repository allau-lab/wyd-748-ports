...
type Player struct {
	...
	ID       uint16
	InWorld  bool
	// ClientABI seleciona a projecao on-wire (748 canonico ou 730 legado).
	ClientABI wire.ClientABI
...
// WithServerList habilita a gestao do serverlist.bin do CLIENT pelo painel
// (comandos serverlist.*): path e o arquivo na pasta do client e gameAddress
// e o host:porta deste servidor, usado para gerar a lista padrao quando o
// arquivo ainda nao existe. Path vazio desliga a feature.
func WithServerList(path, gameAddress string) WorldOption {
	return func(w *World) {
		w.serverListPath = strings.TrimSpace(path)
		w.gameAddress = gameAddress
	}
}
...
type World struct {
	...
	// adminCommands leva operacoes administrativas (console --cli / GUI) para
	// dentro do game-loop: a mesma single-goroutine que processa pacotes garante
	// que nenhuma opcao admin corra em paralelo com o tick.
	adminCommands chan adminJob
	...
	// serverListPath/gameAddress habilitam os comandos serverlist.* do painel
	// (comando WithServerList): escrita do serverlist.bin na pasta do client.
...
func NewWorld(st store.Store, npcs []model.NPCDef, geners []model.NPCGener, catalog model.Catalog, dropRates [model.MaxCarry]int, volatiles model.VolatileCatalog, characterTemplates model.CharacterTemplateFile, terrain model.TerrainMap, options ...WorldOption) (*World, error) {
	...
	w.adminCommands = make(chan adminJob, AdminCallCapacity)
	...
	for {
		...
		if !ok {
			...
			case job := <-w.adminCommands:
				...
				// Backlog admin pendente sai sem esperar o proximo tick.
				w.drainAdminJobs()
				continue
			case <-ticker.C:
				w.tick()

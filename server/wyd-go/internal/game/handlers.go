...
// banStore e a capacidade opcional de suspensao de contas (SQLiteStore).
type banStore interface {
	AccountBan(ctx context.Context, name string) (*store.BanInfo, error)
}
...
func (w *World) onLogin(s *net.Session, pkt []byte) {
	...
	go func() {
		...
		w.commands <- command{s: s, queuedAt: time.Now(), login: &loginResult{
			accountName: accountName, account: acc, err: err, cliver: cliver,
		}}
	}()
}
...
func (w *World) onLoginResult(s *net.Session, result *loginResult) {
	...
	acc := result.account
	// Conta banida e rejeitada antes de qualquer etapa de rede/sessao.
	if banner, ok := w.store.(banStore); ok {
		if info, err := banner.AccountBan(context.Background(), acc.Name); err == nil && info != nil {
			until := "definitivo"
			if info.ExpiresAt > 0 {
				until = time.Unix(info.ExpiresAt, 0).Format("2006-01-02 15:04")
			}
			log.Printf("[#%d] LOGIN banido conta=%q ate %s (%s)", s.ID, acc.Name, until, info.Reason)
			s.Send(wire.MessagePanel("This account is suspended."))
			time.AfterFunc(300*time.Millisecond, s.Close)
			return
		}
	}
	...
	p := &Player{Session: s, Account: acc, CharSlot: -1, ClientABI: wire.ABIFromCliver(result.cliver)}
	w.players[s] = p
	s.Send(characterListPacket(s, p))
	log.Printf("[#%d] char-list enviada (%d personagem(ns)) abi=%d cliver=%d",
		s.ID, characterCount(acc.Chars), p.ClientABI, result.cliver)
}
...
func (w *World) onCreateCharacter(s *net.Session, pkt []byte) {
	...
	if slot < len(p.Account.Chars) && p.Account.Chars[slot].Name != "" {
		log.Printf("[#%d] criacao recusada slot=%d ocupado por %q (pedido nome=%q)",
			s.ID, slot, p.Account.Chars[slot].Name, name)
		s.Send(wire.NewCharacterFail())
		return
	}
...
func (w *World) onEnterWorld(s *net.Session, pkt []byte) {
	...
	// 1) enter-world (STRUCT_MOB completo — tamanho depende do ABI do client)
	clientChar := *ch
	clientChar.Equip = clientEquipProjection(ch)
	s.Send(wire.EnterWorldFor(p.ClientABI, p.ID, uint16(slot), clientChar))
	...
	s.Send(wire.CreateMobWithGuildRankFor(p.ClientABI, p.ID, ch.Name, ch.X, ch.Y, bodyMesh(ch),
...
func (w *World) onAttack(s *net.Session, pkt []byte) {
	...
	if m.HP == 0 {
		...
		w.sendToMobViewProtocol(m, func(observer *Player) []byte {
			...
			return wire.MobHpMpFor(observer.ClientABI, m.ID, m.HP, m.Def.Score.MaxHP,

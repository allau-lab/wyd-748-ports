...
func (f ClientIntegrityFile) Validate() error {
	...
	for i, probe := range f.Probes {
		...
		// filepath.IsAbs/VolumeName sao dependentes do SO hospedeiro: em Linux um
		// caminho "C:\WYD\..." (manifesto criado no source Windows) passa. Rejeita
		// tambem a notacao Windows explicita: drive "X:" e separador invertido.
		windowsStyle := strings.ContainsRune(asset, '\\') ||
			(len(asset) >= 2 && asset[1] == ':' && asset[0] != '/')
		if asset == "" || cleaned == "." || filepath.IsAbs(cleaned) || filepath.VolumeName(cleaned) != "" ||
			cleaned == ".." || strings.HasPrefix(cleaned, ".."+string(filepath.Separator)) || windowsStyle {

...
func TestMessagePanel748Layout(t *testing.T) {
	...
	// O TMScene 7.48 so exibe o aviso se comecar com '^'; o builder adiciona.
	if got := string(b[12 : 12+17]); got != "^Inventario limpo" {
		t.Fatalf("MessagePanel texto=%q (esperado com caret)", got)
	}
	if b[107] != 0 {
		t.Fatal("MessagePanel sem terminador final")
	}
	// Texto ja prefixado nao deve ganhar caret duplo; protocolo '!#' e preservado.
	if string(MessagePanel("^ja tem caret")[12:14]) != "^j" {
		t.Fatal("MessagePanel duplicou o caret")
	}
	if string(DaySync()[12:14]) != "!#" {
		t.Fatal("MessagePanel corrompeu o payload de protocolo !#")
	}
}

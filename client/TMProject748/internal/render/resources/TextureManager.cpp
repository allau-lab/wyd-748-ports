...
TextureManager::TextureManager()
{
	if (g_pTextureManager != nullptr)
		throw std::runtime_error("TextureManager already initialized");
...
void TextureManager::ReleaseNotUsingTexture()
{
	...
	if (m_LastLoadTime + 900 <= dwCurrentTime)
	{
...
#if defined(__SWITCH__)
		// Recarregar do SD + decodificar + subir de novo é o que causa os engasgos.
		const bool keepStatic = WYD_Switch_KeepAssetsResident() != 0;
#else
		const bool keepStatic = false;
#endif
		for (int nIndex = 0; nIndex < MAX_DYNAMIC_TEXTURE; ++nIndex)
		{
		...
		if (keepStatic)
			return;
#if defined(__SWITCH__)
		// Album (~440 MB): mantém UI + efeitos residentes (evita thrash em ícones/FX);
		// libera só modelos/ambiente (maior volume).
#else
		for (int nIndex = 0; nIndex < MAX_UI_TEXTURE; ++nIndex)
		{
...
#ifdef __linux__
	// Persist case-correct paths (UI\\inven01.wyt -> UI/Inven01.wyt) so every
	// later open hits the real filename even if a call bypasses WYD_NormalizePath.
	for (int i = 0; i < static_cast<int>(_countof(m_stUITextureList)); ++i)
	{
		if (m_stUITextureList[i].szFileName[0] == '\0')
			continue;
		const std::string resolved = WYD_NormalizePath(m_stUITextureList[i].szFileName);
		if (!resolved.empty() && resolved != m_stUITextureList[i].szFileName)
		{
			std::snprintf(m_stUITextureList[i].szFileName,
				sizeof(m_stUITextureList[i].szFileName),
				"%s",
				resolved.c_str());
		}
	}
...
int TextureManager::LoadUITexture(int nIndex, int nSrcIndex)
{
	...
	if (handle == -1)
	{
		static int s_uiOpenFailLog = 0;
		if (s_uiOpenFailLog < 32)
		{
			std::fprintf(stderr, "[WYDLINUX] LoadUITexture open fail idx=%d src=%d path='%s'\n",
				nIndex, nSrcIndex, m_stUITextureList[nSrcIndex].szFileName);
			++s_uiOpenFailLog;
		}
		WYD_DiagMissing("tex-ui", m_stUITextureList[nSrcIndex].szFileName);
		return 0;
	}
	...
	delete[] pBuffer;
	{
		static int s_uiDecodeFailLog = 0;
		if (s_uiDecodeFailLog < 32)
		{
			std::fprintf(stderr, "[WYDLINUX] LoadUITexture decode fail idx=%d path='%s'\n",
				nIndex, m_stUITextureList[nSrcIndex].szFileName);
			++s_uiDecodeFailLog;
		}
	}
	return 0;
...
int TextureManager::LoadEffectTexture(int nIndex)
{
	...
	if (handle == -1)
	{
		WYD_DiagMissing("tex-effect", m_stEffectTextureList[nIndex].szFileName);
		return 0;
	}
...
int TextureManager::LoadModelTexture(int nIndex)
{
	...
	if (handle == -1)
	{
		WYD_DiagMissing("tex-model", m_stModelTextureList[nIndex].szFileName);
		return 0;
	}
...
#ifdef __linux__
	for (int i = 0; i < static_cast<int>(_countof(m_stEnvTextureList)); ++i)
	{
		if (m_stEnvTextureList[i].szFileName[0] == '\0')
			continue;
		const std::string resolved = WYD_NormalizePath(m_stEnvTextureList[i].szFileName);
		if (!resolved.empty() && resolved != m_stEnvTextureList[i].szFileName)
		{
			std::snprintf(m_stEnvTextureList[i].szFileName,
				sizeof(m_stEnvTextureList[i].szFileName),
				"%s",
				resolved.c_str());
		}
	}
...
int TextureManager::LoadEnvTexture(int nIndex)
{
	...
	if (handle == -1)
	{
		WYD_DiagMissing("tex-env", m_stEnvTextureList[nIndex].szFileName);
		return 0;
	}

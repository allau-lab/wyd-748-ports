...
int TMMesh::RenderForUI(int nX, int nY, float fAngle, float fScale, DWORD dwColor, int nMultiTex, int nTexOffset, short sLegend)
{
	...
	// Both constants are the visible width of SetMatrixForUI's camera (FOV 0.1 at
	// distance 50 -> height ~5.0) times 4:3 / 5:4. Wider viewports need the same
	// product, otherwise icons drift toward the center as they approach the edges.
	const float fAspect = (float)g_pDevice->m_viewport.Width / (float)g_pDevice->m_viewport.Height;
	float fWidthRatio = 6.68f;
	if (fAspect < 1.26f)
		fWidthRatio = 6.26f;
	else if (fAspect > 1.4f)
		fWidthRatio = 100.0f * tanf(0.05f) * fAspect;
...
int TMMesh::LoadMsa(const char* szFileName)
{
	...
	if (!fp)
	{
		...
		WYD_DiagMissing("msa", szFileName);
		return 0;
	}

...
int MeshManager::InitBoneAnimation()
{
	...
	for (int Count = 0; Count < MAX_BONE_ANIMATION_LIST; ++Count)
	{
		...
		if (handle < 0)
		{
			WYD_DiagMissing("bon", szTemp);
			MeshManager::m_BoneAnimationList[nCount].pBone = nullptr;
			MeshManager::m_BoneAnimationList[nCount].numBone = 0;
			MeshManager::m_BoneAnimationList[nCount].numBoneBytes = 0;
		}
		else
		{
			...
			if (sz < 0)
				sz = 0;
			MeshManager::m_BoneAnimationList[nCount].pBone = (unsigned int*)malloc(sz > 0 ? (size_t)sz : 1);
			if (sz > 0 && MeshManager::m_BoneAnimationList[nCount].pBone)
...
{
#if defined(__SWITCH__)
	if (WYD_Switch_KeepAssetsResident())
		return;
#endif

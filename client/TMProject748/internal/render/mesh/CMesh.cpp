...
int CMesh::Render(int nBright)
{
	...
	// Opaque/'N' skins: keep alphablend off so soft DXT alpha cannot open holes
	// in torsos while ALPHATEST is still used for punch-through textures.
	if (cAlpha == 'N' || cAlpha == 'C')
...
#if defined(WYD_LINUX) || defined(__linux__)
		// Bright-pass / alternate paths may skip SetMaterial and leave c0.w at the
		// D3DCOLOR scale (765), exploding mob bone indices into giants.
		{
			const float fProgress = (float)(g_pTimerManager->GetServerTime() % 10000) / 10000.0f;
			const float fPower = m_pParentSkin ? m_pParentSkin->m_materials.Power : 0.0f;
			D3DXVECTOR4 vIdxScale(1.0f, fPower, fProgress, 3.0f);
			g_pDevice->m_pd3dDevice->SetVertexShaderConstantF(0, (const float*)&vIdxScale, 1);
		}
#endif
...
#if defined(WYD_LINUX) || defined(__linux__)
		// skinmesh*.bin: mul r, vIndices.zyxw, c0.w with c0.w=765 for D3DCOLOR
		// (byte/255*765 = index*3). UBYTE4 already delivers 0..255 integers in
		// .xyzw order after our load-time swizzle patch (.zyxw→.xyzw), so scale=3.
		D3DXVECTOR4 vConst(1.0f, 0.0f, fProgress, 3.0f);
#else
...
#ifdef __linux__
        auto pC2 = D3DXCOLOR(0.55f, 0.55f, 0.55f, 1.0f);
#else

...
int TMGround::LoadTileMap(const char* szFileName)
{
    ...
    if (fp)
    {
        ...
        */
        ...
        // DXVK Native: o difuso do tilemap quase some; emissive mais alto evita
        // o chão preto mesmo com ambient/luz fracos (Win32 sofria menos).
        m_materials.Emissive.r = 0.55f;
        m_materials.Emissive.g = 0.55f;
        m_materials.Emissive.b = 0.55f;
...
{  
    ...
    // Selchar: o piso vem dos meshes do Character.dat; o early-return do
    // tilemap era do campo. Em Linux/DXVK alguns tiles do .trn ainda ajudam
    // a fechar buracos pretos sob o tapete — desenhar o ground.
    // (ESCENE_SELCHAR não retorna mais aqui.)
    ...
    // DXVK Native: fixed-function lighting on the tilemap washes out to black.
    // Use the legacy "voodoo" unlit + vertex-color path on Linux (and real
    // Voodoo cards) so terrain stays readable like the Win32 client.
#ifdef __linux__
    const int bUnlitTilemap = 1;
#else
    const int bUnlitTilemap = (g_pDevice->m_bVoodoo == 1) ? 1 : 0;
#endif
    ...
    if (bUnlitTilemap == 1)
    {
        ...
        g_pDevice->SetTextureStageState(0, D3DTSS_COLOROP, 4); // MODULATE
        g_pDevice->SetTextureStageState(0, D3DTSS_COLORARG1, 2); // TEXTURE
        g_pDevice->SetTextureStageState(0, D3DTSS_COLORARG2, 0); // DIFFUSE
        g_pDevice->SetTextureStageState(1, D3DTSS_COLOROP, 1); // DISABLE
    }
    else
    {
        g_pDevice->SetRenderState(D3DRS_COLORVERTEX, 1);
...
#ifdef __linux__
                const int bUnlitVerts = 1;
#else
                const int bUnlitVerts = (g_pDevice->m_bVoodoo == 1) ? 1 : 0;
#endif
                if (bUnlitVerts == 1)
                {
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        ...
                        // Original packed only RGB; alpha stayed 0. With
                        // D3DRS_ALPHABLENDENABLE left on from SetRenderStateBlock(1),
                        // SRCALPHA makes every tile fully transparent on DXVK.
                        const unsigned int r8 = (unsigned int)(RetColor.r * 255.0f) & 0xFFu;
                        const unsigned int g8 = (unsigned int)(RetColor.g * 255.0f) & 0xFFu;
                        const unsigned int b8 = (unsigned int)(RetColor.b * 255.0f) & 0xFFu;
                        m_vertexVoodoo[i].diffuse = 0xFF000000u | (r8 << 16) | (g8 << 8) | b8;
                    }
                    if (nX < 63 && nY < 63)
                    {

...
int TMSky::FrameMove(unsigned int dwServerTime)
{
    ...
    // Native 70 clips the world too early on DXVK (looks like short DOF). Keep
    // fog for atmosphere but allow terrain/mobs out to a longer far plane.
    g_ClipFar = 320.0f;
#endif
    ...
    else
    {
...
#if defined(WYD_LINUX) || defined(__linux__)
        // Vertex-shader fog still fades meshes even with D3DRS_FOGENABLE off.
        g_pDevice->m_fFogStart *= 1.75f;
        g_pDevice->m_fFogEnd *= 1.75f;
#endif
        if (m_nState / 10 == 1)
        {
        ...
        else
        {
            ...
            {
                if (g_pDevice->m_colorLight.r < 0.85f) g_pDevice->m_colorLight.r = 0.85f;
                if (g_pDevice->m_colorLight.g < 0.85f) g_pDevice->m_colorLight.g = 0.85f;
                if (g_pDevice->m_colorLight.b < 0.85f) g_pDevice->m_colorLight.b = 0.85f;
                g_pDevice->m_colorLight.a = 1.0f;
            }
#endif
        }
...
{
    // Always publish fog/clear RGB even if the sky mesh failed to load — otherwise
    // DXVK keeps SelectServer's black clear and the field becomes a black void.
    if (!(nState / 10) && g_pDevice != nullptr)
    {
        const int idx = nState % 10;
        const unsigned int dwR = static_cast<unsigned char>(m_dwR[idx]);
        const unsigned int dwG = static_cast<unsigned char>(m_dwG[idx]);
        const unsigned int dwB = static_cast<unsigned char>(m_dwB[idx]);
        g_pDevice->m_dwClearColor = dwB | (dwG << 8) | (dwR << 16);
    }

...
public:
	...
	int CanAttackHuman(TMHuman* pOver);
	...
	void GameAuto();
	void UpdateGroundTeleportPrompt(unsigned int dwServerTime);
	void UpdateStatusPointHoldRepeat(unsigned int dwServerTime);
	void UpdateClickTargetVisuals(unsigned int dwServerTime);
	void SetClickTargetAtPick(const D3DXVECTOR3& pick);
	// Ground move / repath (shared by classic LMB and LoL RMB).
	// holdRepeat=true skips repath when the destination cell is unchanged.
	void IssueMoveToPick(const D3DXVECTOR3& pick, bool holdRepeat = false);
	void IssueStickStep(float dirX, float dirY, bool stop);
...
public:
	...
	// Last cell issued by IssueMoveToPick (LoL hold repath debounce).
	IVector2 m_vecLastIssueMoveCell;
	unsigned int m_dwLastIssueMoveTime;
	int m_nVillage;
	...
	unsigned int m_dwClbuttonHoldStart;
	int m_bClbutton;
	...
	SButton* m_pSwitchCtlTab = nullptr;
	SButton* m_pSwitchMacroTab = nullptr;
	SButton* m_pSwitchCtlUp = nullptr;
	SButton* m_pSwitchCtlDown = nullptr;
	SListBox* m_pSwitchCtlList = nullptr;
	SText* m_pSwitchMacroHud = nullptr;
	unsigned int m_dwSwitchCtlRev = 0;
	int m_nSwitchCtlPage = -1;
	void SwitchControlsTabInit();
	void SwitchControlsTabShow(int bShow, int page = 0);
	void SwitchControlsTabTick();
	int SwitchControlsTabEvent(unsigned int idwControlID, unsigned int idwEvent);
	void SwitchOpenMacroTab();
#endif

...
extern "C" void WYD_Linux_SetBackbuffer(int w, int h);
#endif
...
HRESULT NewApp::Initialize(HINSTANCE hInstance, int nFull)
{
...
#ifdef WYD_LINUX
	// A direct client launch may override the RES entry without modifying
	// config.txt: `./project --resolution=1280x720`. Match only the native
	// 7.48 table so the render, UI ratios and D3D9 backbuffer stay identical.
	if (const char* resolution = std::getenv("WYD_RESOLUTION"))
	{
		int requestedWidth = 0;
		int requestedHeight = 0;
		if (std::sscanf(resolution, "%dx%d", &requestedWidth, &requestedHeight) == 2)
		{
			for (int i = 0; i < static_cast<int>(_countof(stResList)); ++i)
			{
				if (static_cast<int>(stResList[i].dwWidth) == requestedWidth &&
					static_cast<int>(stResList[i].dwHeight) == requestedHeight)
				{
					nResIndex = i + 1;
					break;
				}
	...
	// O backbuffer GLES segue a NWindow (1280x720); layout, picking e toque
	// precisam da mesma grade ou painéis de borda somem e o clique desloca.
	nResIndex = 4;
#endif
	...
	{
		int lolControls = 0;
		if (WYD748_LoadOptionalClientInt("config.txt", "LOL_CONTROLS", &lolControls))
			g_bLolControls = lolControls != 0;
	}

	// Keep the touch input bridge available on Linux so the right-side Main Menu
	// can enable the HUD, but the HUD itself starts hidden (TouchControlsUI).
	g_bTouchUi = 0;
#if defined(WYD_LINUX) || defined(__linux__) || defined(__aarch64__) || defined(__arm64__)
	g_bTouchUi = 1;
#endif
	{
		int touchUi = 0;
		if (WYD748_LoadOptionalClientInt("config.txt", "TOUCH_UI", &touchUi))
			g_bTouchUi = touchUi != 0;
	}

	// Opções gráficas adicionais não fazem parte do bloco binário legado. São
	// opcionais para preservar compatibilidade com clientes antigos.
	int videoValue = 0;
	if (WYD748_LoadOptionalClientInt("config.txt", "ANISOTROPIC", &videoValue))
		g_nAnisotropic = std::max(0, std::min(videoValue, 16));
	if (WYD748_LoadOptionalClientInt("config.txt", "VSYNC", &videoValue))
		g_bVsync = videoValue != 0;
	if (WYD748_LoadOptionalClientInt("config.txt", "MIPMAP", &videoValue))
		nMipMap = std::max(0, std::min(videoValue, 50));
	if (WYD748_LoadOptionalClientInt("config.txt", "ANTIALIAS", &videoValue))
		g_nAntialias = std::max(0, std::min(videoValue, 8));
	if (WYD748_LoadOptionalClientInt("config.txt", "REFLECTION", &videoValue))
		g_nReflection = videoValue != 0;
	if (WYD748_LoadOptionalClientInt("config.txt", "DEBUG_STATS", &videoValue))
		#if defined(__SWITCH__)
		g_bDebugMsg = 0;
#else
		g_bDebugMsg = videoValue != 0;
#endif
#ifdef WYD_LINUX
	// O backend DXVK-native lê esta variável ao criar o swap chain.
	::setenv("WYD_NO_VSYNC", g_bVsync ? "0" : "1", 1);
#endif

#if defined(WYD_LINUX) || defined(__linux__)
	TouchControlsUI::Instance().SetFeatureEnabled(g_bTouchUi);
	// Touch HUD assumes LoL-style cast/move separation.
	if (g_bTouchUi)
		g_bLolControls = 1;
#endif
...
#if defined(WYD_LINUX) || defined(__linux__)
	// Preservar defaults de qualidade, mas permitir reduzir o custo em GPUs
	// integradas/ARM64 pelo config.txt.
	if (g_nAntialias < 0)
		g_nAntialias = 0;
	if (g_nReflection < 0)
		g_nReflection = 0;
	...
	if (nCursor == 0)
	{
		// Cursor software: esconder o ponteiro do compositor desde o boot.
		SetCursor(nullptr);
		ShowCursor(FALSE);
	}
	else if (nCursor == 2)
...
#ifdef WYD_LINUX
	// RES ja define a resolução real do render e da janela. Não substitua esses
	// valores pelo tamanho físico detectado pelo Wayland: isso aplicava escalas
	// X/Y diferentes ao layout 7.48 e deslocava textos, campos e texturas.
	// Para trocar a resolução, altere [RES] em config.txt e reinicie o cliente.
	WYD_Linux_SetBackbuffer(static_cast<int>(m_dwScreenWidth), static_cast<int>(m_dwScreenHeight));
#endif
...
DWORD NewApp::Run()
{
	...
	{
#ifdef WYD_LINUX
		// WSAAsyncSelect é poll-driven: notifica WM_USER+100 com lParam=FD_READ.
		WYD_Linux_PollSockets();
#endif
		int bGotMsg = 0;
		if (!m_bActive)
...
#ifdef WYD_LINUX
		if (m_pSocketManager)
			WYD_Linux_SocketAckRead(m_pSocketManager->Sock);
#endif
	}
	break;
	case WM_USER + 101:
...
{
#if defined(WYD_LINUX) || defined(__linux__)
	// Wayland/SDL windowed path can create any size; EnumDisplaySettings only
	// lists monitor modes and was forcing a silent fallback to 800x600 when
	// [RES] 3 (1024x768) was missing from the list.
	(void)x;
	(void)y;
	(void)bpp;
	return true;

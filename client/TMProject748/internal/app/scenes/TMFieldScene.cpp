...
#if defined(WYD_LINUX) || defined(__linux__)
	...
	: TMScene()
{
	...
	m_cGuildOnOff = 1;
	m_bMoveing = 0;
	m_vecLastIssueMoveCell = IVector2(0, 0);
	m_dwLastIssueMoveTime = 0;
	m_nDustCount = 0;
	m_pSysMsgList = 0;
	...
	// Bag_View / Japanese bag-page controls are 7.59-only; leave them null so
	// the enter-world UpdateScore path can no-op instead of dereferencing junk.
	m_pInvPageBtn1 = nullptr;
	m_pInvPageBtn2 = nullptr;
	m_pInvPageBtn3 = nullptr;
	m_pInvPageBtn4 = nullptr;
	m_pJPNBag_Day1 = nullptr;
	m_pJPNBag_Day2 = nullptr;
	m_pGridInv = nullptr;
	...
	// Quick-slot shortcuts (Q/W/E/R/T) are bound only in the full 7.59
	// initializer. Compat FieldScene2 must keep these null until FindControl.
	memset(m_pQuick_Sloat, 0, sizeof(m_pQuick_Sloat));
	memset(m_pQuick_SloatQ, 0, sizeof(m_pQuick_SloatQ));
...
void SetMinimapPos()
{
	...
	if (fpMinimap)
	{
		...
		{
			if (fscanf(fpMinimap, "%d %d %d %d %d %31s", &g_MinimapPos[index].nX, &g_MinimapPos[index].nY, &Color, &g_MinimapPos[index].nCX, &g_MinimapPos[index].nCY, g_MinimapPos[index].szTarget) == -1)
				break;
...
void TMFieldScene::UpdateCompatScoreUI()
{
	...
	auto setNumber = [this](unsigned int controlID, long long value)
	{
		...
		{
			char text[64]{};
			sprintf_s(text, WYD_FMT_I64, value);
	...
	const long long levelSpan = std::max(levelEnd - levelStart, 1LL);
	if (levelProgress < 0)
		levelProgress = 0;
	if (levelProgress > levelSpan)
		levelProgress = levelSpan;
	const long long quarterSpan = std::max(levelSpan >> 2, 1LL);
	int activeQuarter = 4;
		...
		m_pControlContainer->FindControl(1171)))
	{
		const int nativeQuarterMax = static_cast<int>(
			std::min(quarterSpan, 0x7FFFFFFFLL));
		const int nativeQuarterProgress = static_cast<int>(
			std::min(quarterProgress, static_cast<long long>(nativeQuarterMax)));
	...
	// Control 572 is the native "Skill Points" counter on root 1905. Leaving it
	// untouched made the skill window look like the character had zero points
	// and blocked confident purchases at the skill master.
	setNumber(TMT_SKILL_BONUS, g_pObjectManager->m_stMobData.CurrentScore.SkillPts);
	if (m_pSkBonus)
	{
		char skillPtsText[32]{};
		sprintf_s(skillPtsText, "%u", g_pObjectManager->m_stMobData.CurrentScore.SkillPts);
		m_pSkBonus->SetText(skillPtsText, 0);
	}
	...
	// Learned-skill cells on root 1905 (controls 547..570). Without this the
	// Skill window looks empty even after buying skills from the master.
	switch (characterClass)
	{
	case 0:
		setCompatText(TMT_SKILL_SEC1_C, g_pMessageStringTable[107]);
		setCompatText(TMT_SKILL_SEC2_C, g_pMessageStringTable[108]);
		setCompatText(TMT_SKILL_SEC3_C, g_pMessageStringTable[109]);
		break;
	case 1:
		setCompatText(TMT_SKILL_SEC1_C, g_pMessageStringTable[110]);
		setCompatText(TMT_SKILL_SEC2_C, g_pMessageStringTable[111]);
		setCompatText(TMT_SKILL_SEC3_C, g_pMessageStringTable[112]);
		break;
	case 2:
		setCompatText(TMT_SKILL_SEC1_C, g_pMessageStringTable[113]);
		setCompatText(TMT_SKILL_SEC2_C, g_pMessageStringTable[114]);
		setCompatText(TMT_SKILL_SEC3_C, g_pMessageStringTable[115]);
		break;
	default:
		setCompatText(TMT_SKILL_SEC1_C, g_pMessageStringTable[133]);
		setCompatText(TMT_SKILL_SEC2_C, g_pMessageStringTable[134]);
		setCompatText(TMT_SKILL_SEC3_C, g_pMessageStringTable[135]);
		break;
	}
	...
	for (int i = 0; i < 24; ++i)
	{
		...
		const unsigned int dwBit = 1u << i;
		if ((dwBit & static_cast<unsigned int>(pMobData->LearnedSkill[0])) != dwBit)
		{
			m_pSkillSecGrid[i]->Empty();
			continue;
		}

		const int skillItemIndex = 24 * characterClass + i + 5000;
		auto pSkillItem = m_pSkillSecGrid[i]->GetItem(0, 0);
		if (pSkillItem && pSkillItem->GetItem() && pSkillItem->GetItem()->sIndex == skillItemIndex)
			continue;

		m_pSkillSecGrid[i]->Empty();
		auto pStructItem = new STRUCT_ITEM{};
		pStructItem->sIndex = static_cast<short>(skillItemIndex);
...
void TMFieldScene::InitializeCompatSkillBelts()
{
	...
	// Q/W/E/R/T → UseQuickSloat. FieldScene2 may only ship a subset of the
	// modern 66560..66564 grids; bind what exists and leave the rest null so
	// UseQuickSloat can no-op instead of crashing.
	static const unsigned int kQuickSlotIds[5] = {
		Q_QUICK_SLOAT1, Q_QUICK_SLOAT2, Q_QUICK_SLOAT3, Q_QUICK_SLOAT4, Q_QUICK_SLOAT5
	};
	static const TMEGRIDTYPE kQuickSlotTypes[5] = {
		TMEGRIDTYPE::GRID_QUICKSLOAT1, TMEGRIDTYPE::GRID_QUICKSLOAT2,
		TMEGRIDTYPE::GRID_QUICKSLOAT3, TMEGRIDTYPE::GRID_QUICKSLOAT4,
		TMEGRIDTYPE::GRID_QUICKSLOAT5
	};
	int quickBound = 0;
	for (int i = 0; i < 5; ++i)
	{
		m_pQuick_Sloat[i] = static_cast<SGridControl*>(
			m_pControlContainer->FindControl(kQuickSlotIds[i]));
		if (m_pQuick_Sloat[i])
		{
			m_pQuick_Sloat[i]->m_eGridType = kQuickSlotTypes[i];
			++quickBound;
		}
	}

	WYD748_DiagnosticsLog("compat native skill grids bound learned=%p belt2=%p belt3=%p quick=%d/5\r\n",
		m_pGridSkillBelt, m_pGridSkillBelt2, m_pGridSkillBelt3, quickBound);

	// FieldScene2 skill window (root 1905): remaining points + 24 learned-skill
	// cells. The imported 7.59 IDs (65600 / 65574+) do not exist here, so the
	// compat HUD must bind the original controls or the panel stays empty and
	// SkillPts never appear.
	m_pSkBonus = static_cast<SText*>(m_pControlContainer->FindControl(TMT_SKILL_BONUS));
	m_pSkillSec1 = static_cast<SText*>(m_pControlContainer->FindControl(TMT_SKILL_SEC1_C));
	m_pSkillSec2 = static_cast<SText*>(m_pControlContainer->FindControl(TMT_SKILL_SEC2_C));
	m_pSkillSec3 = static_cast<SText*>(m_pControlContainer->FindControl(TMT_SKILL_SEC3_C));
	int skillSecBound = 0;
	for (int i = 0; i < 24; ++i)
	{
		...
		if (m_pSkillSecGrid[i])
		{
			m_pSkillSecGrid[i]->m_bSelectEnable = 1;
			++skillSecBound;
		}
	}
	WYD748_DiagnosticsLog("compat skill panel bonus=%p sections=%p/%p/%p grids=%d/24\r\n",
		m_pSkBonus, m_pSkillSec1, m_pSkillSec2, m_pSkillSec3, skillSecBound);
}
...
int TMFieldScene::OnMouseEventCompat(unsigned int dwFlags, unsigned int wParam, int nX, int nY)
{
	...
	// Dead / town-return: the full OnMouseEvent shows MessageBox(11) on click.
	// Compat previously returned here without opening it, so the next click fell
	// through the System menu (635=quit) and looked like "return to city" exits.
	const bool bDead = g_pObjectManager->m_stMobData.CurrentScore.CurHP <= 0
		|| m_pMyHuman->m_cDie == 1;
	if (bDead)
	{
		if (dwFlags == WM_LBUTTONDOWN && m_pMessageBox && !m_pMessageBox->IsVisible()
			&& !m_pMyHuman->m_sFamCount)
		{
			POINT pt{};
			pt.x = static_cast<int>(m_pMyHuman->m_vecPosition.x);
			pt.y = static_cast<int>(m_pMyHuman->m_vecPosition.y);
			if (!m_pMyHuman->IsInTown() || PtInRect(&rectTownInCastle, pt) == 1)
			{
				if (m_pSystemPanel && m_pSystemPanel->IsVisible())
	...
	// LoL controls: RMB = move/repath; LMB = skill (if selected) or attack/merchant.
	// Classic RMB SkillUse is replaced by LMB when a short-skill slot is armed.
	if (g_bLolControls)
	{
		if (dwFlags == 516 && !(wParam & 8))
		{
			// Hold-repeat is flagged via MK_MBUTTON (unused for LoL move) from
			// EventTranslator so we can debounce without changing the ABI.
			const bool holdRepeat = (wParam & MK_MBUTTON) != 0;
			IssueMoveToPick(pick, holdRepeat);
			return 1;
		}
		if (dwFlags == 514)
		{
		...
		// Selected short-skill (1–0 / belt) casts on LMB — same as classic RMB.
		const int shortSlot = g_pObjectManager->m_cSelectShortSkill;
		if (shortSlot >= 0 && shortSlot < 20)
		{
			const char rawSkill = g_pObjectManager->m_cShortSkill[shortSlot];
			if (rawSkill != -1)
			{
				const int skillHandled = SkillUse(nX, nY, pick, serverTime, 1, 0);
				if (skillHandled)
					return 1;
			}
		}
...
int TMFieldScene::InitializeCompatFieldScene()
{
	...
	if (m_pControlContainer)
	{
		...
		if (m_pHelpPanel)
		{
			...
			if (m_pHelpList[0] && !m_pHelpList[0]->m_nNumItem)
				LoadMsgText(m_pHelpList[0], (char*)"UI\\interface.txt");
			if (m_pHelpList[1] && !m_pHelpList[1]->m_nNumItem)
				LoadMsgText(m_pHelpList[1], (char*)"UI\\command.txt");
			if (m_pHelpList[2] && !m_pHelpList[2]->m_nNumItem)
				LoadMsgText(m_pHelpList[2], (char*)"UI\\etc.txt");
#if defined(__SWITCH__)
			SwitchControlsTabInit();
#endif
		}
		...
		for (int affectIndex = 0; affectIndex < 16; ++affectIndex)
		{
			...
			// FieldScene2 has no 7.59 duration labels (90432+). Create them so
			// Affect_Main can show remaining time under each buff icon.
			m_pAffect[affectIndex] = new SText(-1, "", 0xFFFFFFFFu, 0.0f, 0.0f, 40.0f, 16.0f,
				0, 0x77777777u, 0u, 0u);
			if (m_pAffect[affectIndex])
			{
				m_pAffect[affectIndex]->SetControlID(12822u + affectIndex);
				m_pAffect[affectIndex]->SetVisible(0);
				m_pControlContainer->AddItem(m_pAffect[affectIndex]);
			}
			m_pAffectL[affectIndex] = nullptr;
		}
		...
		if (m_pDescPanel)
		{
			...
			// FieldScene2 uses solid texture -2 with 0x77000000 (too faint on
			// inventory chrome). Use a translucent slate plate — dense enough
			// to read over UI, light enough that name/param text stays bright.
			if (auto* pPanel = static_cast<SPanel*>(m_pDescPanel))
			{
				if (pPanel->m_GCPanel.nTextureSetIndex == -2)
					pPanel->m_GCPanel.dwColor = 0xCC2A2A36u;
			}
			m_pDescPanel->SetAlwaysOnTop(1);
		}
	...
	if (m_pSky)
	{
		// Daytime outdoor sky must be visible; leaving it hidden + a black clear
		// produces the pitch-black void behind Melinda on DXVK.
		m_pSky->m_bVisible = 1;
		AddChild(m_pSky);
	}
	// Full InitializeScene sets these before the first field frame. Without them
	// the DXVK path keeps selchar back-light / ambient and the terrain stays
	// nearly black (diffuse lighting contributes almost nothing).
	if (g_pDevice)
	{
		...
		g_pDevice->m_colorBackLight.a = 1.0f;
		g_pDevice->m_bFog = 1;
		// Daytime fog/clear before first FrameMove (SelectServer left this at 0).
		g_pDevice->m_dwClearColor = 0x001C78BDu;
		g_pDevice->m_dwActualClearColor = 0xFF335599u;
	}
	if (m_pSky)
	...
	// Marcador de clique no chão (anel azul/vermelho). O init completo cria esses
	// efeitos; sem eles o compat não mostra feedback ao clicar para andar.
	if (m_pEffectContainer && !m_pTarget1)
	{
	...
	WYD748_DiagnosticsLog("compat field: before InitCameraView\r\n");
	InitCameraView();
	WYD748_DiagnosticsLog("compat field: before SetCameraView\r\n");
	SetCameraView();
	// Full path calls SetWeather after focusing the player; without it the sky
	// never leaves state 0 lighting transitions that FrameMove expects.
	SetWeather(g_nWeather);
	WYD748_DiagnosticsLog("compat field: before UpdateCompatScoreUI\r\n");
	UpdateCompatScoreUI();
	NewCCMode();
	WYD748_DiagnosticsLog("compat field: InitializeCompatFieldScene OK player=%s at (%d,%d) ccPhys=%p ccMag=%p\r\n",
		m_pMyHuman->m_szName, pMobData->HomeTownX, pMobData->HomeTownY,
		m_pNativeCCPhysicalBtn, m_pNativeCCMagicBtn);
	return 1;
}
...
int TMFieldScene::InitializeScene()
{
	...
	if (m_pKingDomGuild_C)
	{
		m_pKingDomGuild_C->m_bSelected = 1;
		m_pKingDomGuild_C->SetVisible(0);
	}
	if (m_pChatGeneral_C) m_pChatGeneral_C->m_bSelected = 0;
	if (m_pChatParty_C) m_pChatParty_C->m_bSelected = 0;
	if (m_pChatWhisper_C) m_pChatWhisper_C->m_bSelected = 0;
	if (m_pChatGuild_C) m_pChatGuild_C->m_bSelected = 0;
	if (m_pChatGeneral) m_pChatGeneral->m_bSelected = 1;
	if (m_pChatParty) m_pChatParty->m_bSelected = 1;
	if (m_pChatWhisper) m_pChatWhisper->m_bSelected = 1;
	if (m_pChatGuild) m_pChatGuild->m_bSelected = 1;
	if (m_pKingDomGuild) m_pKingDomGuild->m_bSelected = 0;
	if (m_pChatList && m_pChatList->m_pScrollBar)
		m_pChatList->m_pScrollBar->SetVisible(1);
	if (m_pChatListnotice)
	{
		// Bug original: três writes em m_nPosX (a 2ª era Y * HeightRatio).
		m_pChatListnotice->SetPos(
			BASE_ScreenResize(490.0f),
			440.0f * RenderDevice::m_fHeightRatio);
		if (m_pChatListnotice->m_pScrollBar)
  ...
  if(g_pItemList[pMobData->Equip[4].sIndex].nPos == 16)
	...
	g_pObjectManager->m_pCamera->m_fMaxCamLen = 20.0f; // native 7.48 coverage limit
	if (m_pPartyPanel)
	{
	...
	else
	{
		...
		for (int ih = 16; ih < 32; ++ih)
		{
			...
			m_pAffect[ih] = nullptr;
			m_pAffectL[ih] = nullptr;
		}
		for (int ih = 0; ih < 32; ++ih)
...
int TMFieldScene::OnControlEvent(unsigned int idwControlID, unsigned int idwEvent)
{
	...
	if (dwServerTime < m_dwLastRelo + 6000)
		return 1;
#if defined(__SWITCH__)
	if (SwitchControlsTabEvent(idwControlID, idwEvent))
		return 1;
#endif
	...
	if (m_bCompatFieldScene)
	{
		...
		case 635: // native System: quit game
		{
			...
			// While dead, never start quit/logout from System — town return is
			// MessageBox message 11 (control 4617), not button 635.
			if (m_pMyHuman && (m_pMyHuman->m_cDie == 1
				|| g_pObjectManager->m_stMobData.CurrentScore.CurHP <= 0))
			{
				if (m_pSystemPanel)
					m_pSystemPanel->SetVisible(0);
				if (m_pMessageBox && !m_pMessageBox->IsVisible())
				{
					m_pMessageBox->SetMessage(g_pMessageStringTable[27], 11u, 0);
					m_pMessageBox->SetVisible(1);
				}
	...
	if (idwControlID == B_CHATLIST_LIGHT)
	{
		...
		case 3:
#if defined(__SWITCH__)
			// No Switch o preto cheio cobre o mundo atrás do chat; volta ao transparente.
	...
	// Compat 7.48 dispara IDs nativos (1073…); cena moderna usa 65716….
	if (idwControlID == 65716 || idwControlID == 1073)
	{
		if (m_pMyHuman->m_cDie == 1)
			return 1;
		...
		--g_pObjectManager->m_stMobData.CurrentScore.StatusPts;
		m_pMyHuman->m_stScore.StatusPts = g_pObjectManager->m_stMobData.CurrentScore.StatusPts;
		++g_pObjectManager->m_stMobData.CurrentScore.Str;
		++m_pMyHuman->m_stScore.Str;
		return 1;
	}
	if (idwControlID == 65719 || idwControlID == 1076)
	{
		if (m_pMyHuman->m_cDie == 1)
			return 1;
		if (g_pObjectManager->m_stMobData.CurrentScore.StatusPts == 0)
		...
		--g_pObjectManager->m_stMobData.CurrentScore.StatusPts;
		m_pMyHuman->m_stScore.StatusPts = g_pObjectManager->m_stMobData.CurrentScore.StatusPts;
		++g_pObjectManager->m_stMobData.CurrentScore.Int;
		++m_pMyHuman->m_stScore.Int;
		return 1;
	}
	if (idwControlID == 65722 || idwControlID == 1079)
	{
		if (m_pMyHuman->m_cDie == 1)
			return 1;
		if (g_pObjectManager->m_stMobData.CurrentScore.StatusPts == 0)
		...
		--g_pObjectManager->m_stMobData.CurrentScore.StatusPts;
		m_pMyHuman->m_stScore.StatusPts = g_pObjectManager->m_stMobData.CurrentScore.StatusPts;
		++g_pObjectManager->m_stMobData.CurrentScore.Dex;
		++m_pMyHuman->m_stScore.Dex;
		return 1;
	}
	if (idwControlID == 65725 || idwControlID == 1088)
	{
		if (m_pMyHuman->m_cDie == 1)
			return 1;
		if (g_pObjectManager->m_stMobData.CurrentScore.StatusPts == 0)
		...
		--g_pObjectManager->m_stMobData.CurrentScore.StatusPts;
		m_pMyHuman->m_stScore.StatusPts = g_pObjectManager->m_stMobData.CurrentScore.StatusPts;
		++g_pObjectManager->m_stMobData.CurrentScore.Con;
		++m_pMyHuman->m_stScore.Con;
		return 1;
	}
	if (idwControlID == 65754 || idwControlID == 1139)
	{
		if (m_pMyHuman->m_cDie == 1)
			return 1;
		...
		if (pMobData->CurrentScore.Mastery[0] < totalSpecial)
		{
			...
			stApplyBonus.Detail = 0;
			SendPacket({reinterpret_cast<MSG_STANDARD*>(&stApplyBonus)->Type, reinterpret_cast<char*>(&stApplyBonus), sizeof(stApplyBonus)});
			--g_pObjectManager->m_stMobData.CurrentScore.MasterPts;
			m_pMyHuman->m_stScore.MasterPts = g_pObjectManager->m_stMobData.CurrentScore.MasterPts;
			++pMobData->CurrentScore.Mastery[0];
			++m_pMyHuman->m_stScore.Mastery[0];
			return 1;
		}
	...
	if (idwControlID == 65757 || idwControlID == 1142)
	{
		if (m_pMyHuman->m_cDie == 1)
			return 1;
		if (g_pObjectManager->m_stMobData.CurrentScore.MasterPts == 0)
		...
		if (pMobData->CurrentScore.Mastery[1] < totalSpecial)
		{
			...
			--g_pObjectManager->m_stMobData.CurrentScore.MasterPts;
			m_pMyHuman->m_stScore.MasterPts = g_pObjectManager->m_stMobData.CurrentScore.MasterPts;
			++pMobData->CurrentScore.Mastery[1];
			++m_pMyHuman->m_stScore.Mastery[1];
			return 1;
		}
		m_pMessagePanel->SetMessage(g_pMessageStringTable[39], 2000);
		m_pMessagePanel->SetVisible(1, 1);
	...
	if (idwControlID == 65760 || idwControlID == 1145)
	{
		if (m_pMyHuman->m_cDie == 1)
			return 1;
		if (g_pObjectManager->m_stMobData.CurrentScore.MasterPts == 0)
		...
		if (pMobData->CurrentScore.Mastery[2] < totalSpecial)
		{
			...
			--g_pObjectManager->m_stMobData.CurrentScore.MasterPts;
			m_pMyHuman->m_stScore.MasterPts = g_pObjectManager->m_stMobData.CurrentScore.MasterPts;
			++pMobData->CurrentScore.Mastery[2];
			++m_pMyHuman->m_stScore.Mastery[2];
			return 1;
		}
		m_pMessagePanel->SetMessage(g_pMessageStringTable[39], 2000);
		m_pMessagePanel->SetVisible(1, 1);
	...
	if (idwControlID == 65763 || idwControlID == 1154)
	{
		if (m_pMyHuman->m_cDie == 1)
			return 1;
		if (g_pObjectManager->m_stMobData.CurrentScore.MasterPts == 0)
		...
		if (pMobData->CurrentScore.Mastery[3] < totalSpecial)
		{
			...
			--g_pObjectManager->m_stMobData.CurrentScore.MasterPts;
			m_pMyHuman->m_stScore.MasterPts = g_pObjectManager->m_stMobData.CurrentScore.MasterPts;
			++pMobData->CurrentScore.Mastery[3];
			++m_pMyHuman->m_stScore.Mastery[3];
			return 1;
		}
		m_pMessagePanel->SetMessage(g_pMessageStringTable[39], 2000);
		m_pMessagePanel->SetVisible(1, 1);
	...
	// Confirm do pergaminho/portal (12546). A condição estava invertida (!=)
	// e o OK nunca enviava o UseItem após o cast de 5s.
...
int TMFieldScene::FrameMove(unsigned int dwServerTime)
{
	...
	if (m_bCompatFieldScene)
	{
		...
		// Pads de chão (attr 0x10), C.C e hold de pontos viviam só no FrameMove
		// completo — o early-return do compat deixava teleporte/quests-tick mortos.
		UpdateGroundTeleportPrompt(dwServerTime);
		UpdateStatusPointHoldRepeat(dwServerTime);
		UpdateClickTargetVisuals(dwServerTime);
		GameAuto();
		return 1;
	}
...
int TMFieldScene::MouseClick_NPC(int nX, int nY, D3DXVECTOR3 vec, unsigned int dwServerTime)
{
	...
	{
		if (m_pMyHuman && dwServerTime - m_dwLastMouseDownTime > 1000 &&
	...
	{
		if (m_pShopPanel && !m_pShopPanel->IsVisible() && m_pGridShop)
		{
...
int TMFieldScene::SkillUse(int nX, int nY, D3DXVECTOR3 vec, unsigned int dwServerTime, int bMoving, TMHuman* pTarget)
{
	if (m_pPotalPanel && m_pPotalPanel->m_bVisible == 1)
		return 0;
	if (g_pEventTranslator && g_pEventTranslator->m_bAlt == 1)
		return 0;
	if (!m_pMyHuman)
		return 0;
	if (m_pMyHuman->m_cHide == 1)
	...
	if (nDistance > nMobAttackRange || nDX != nTX || nDY != nTY)
	{
		...
		{
			if (m_pMyHuman->BlocksMoveDuringAttackSwing(dwServerTime, g_pEventTranslator && g_pEventTranslator->button[0]))
				return 1;
	...
	if (cSkillIndex == 16 || cSkillIndex == 12 || cSkillIndex == 28)
	{
		...
		for (; pNode; pNode = (TMHuman*)pNode->m_pNextLink)
		{
...
// Regras de alvo de MobAttack (clique) + MoveAttack (auto-ataque), sem hover nem
// cooldown: o macro de toque/gamepad só escolhe alvos que o ataque nativo aceitaria.
// Qualquer mudança nas regras de PK desses dois caminhos precisa ser espelhada aqui.
int TMFieldScene::CanAttackHuman(TMHuman* pOver)
{
	if (!pOver || !m_pMyHuman || pOver == m_pMyHuman)
		return 0;
	if (pOver->m_cDeleted || pOver->m_cDie || pOver->m_cHide == 1)
		return 0;
	if (m_stAutoTrade.TargetID == m_pMyHuman->m_dwID)
		return 0;

	const int IsTargetUser = pOver->m_dwID > 0 && pOver->m_dwID < 1000;

	// MobAttack — bloco pOver
	...
	if (pOver->m_usGuild && (m_pMyHuman->m_usGuild == pOver->m_usGuild || g_pObjectManager->m_usAllyGuild == pOver->m_usGuild))
		return 0;
	if (IsTargetUser && (!pOver->IsInPKZone() || !m_pMyHuman->IsInPKZone()))
		return 0;
	if (m_pMyHuman->m_cCantAttk)
		return 0;
	if (!TMFieldScene::m_bPK && pOver->m_cSummons == 1)
	{
		const bool SameMantua = m_pMyHuman->m_cMantua > 0 && m_pMyHuman->m_cMantua == pOver->m_cMantua;
		if (pOver->m_usGuild && g_pObjectManager->m_usWarGuild != pOver->m_usGuild && !g_bCastleWar)
			return 0;
		if (!pOver->m_usGuild && (!g_bCastleWar || SameMantua))
			return 0;
	}

	// MobAttack — bloco de ataque
	if (!IsTargetUser)
	{
		if (pOver->IsMerchant())
			return 0;
		if ((pOver->m_stScore.Merchant & 0xF) == 15)
		{
			if (!m_pMyHuman->m_cMantua || m_pMyHuman->m_cMantua == 3)
				return 0;
			if (pOver->m_cMantua > 0 && pOver->m_cMantua == m_pMyHuman->m_cMantua)
				return 0;
		}
	}
	if (pOver->m_TradeDesc[0])
	...
	if (!(!IsTargetUser && !pOver->m_bParty && !pOver->m_cSummons || pOver->IsInPKZone()))
		return 0;
	if (!m_pMyHuman->m_cMantua && IsTargetUser && pOver->IsInPKZone() && !TMFieldScene::m_bPK &&
		g_pObjectManager->m_usWarGuild != pOver->m_usGuild)
		return 0;
	if (!TMFieldScene::m_bPK && IsTargetUser)
	{
		if (g_bCastleWar == 1 && m_pMyHuman->m_cMantua == pOver->m_cMantua)
			return 0;
		if (!g_bCastleWar && (g_pObjectManager->m_usWarGuild != pOver->m_usGuild || m_pMyHuman->m_cMantua == pOver->m_cMantua))
			return 0;
	}
	if (!TMFieldScene::m_bPK && pOver->m_cSummons == 1 && g_pObjectManager->m_usWarGuild != pOver->m_usGuild &&
		pOver->m_usGuild && !pOver->IsInCastleZone())
		return 0;
	if (!TMFieldScene::m_bPK && g_bCastleWar > 0 && m_pMyHuman->m_cMantua > 0 && m_pMyHuman->m_cMantua == pOver->m_cMantua)
		return 0;

	const int mx = (int)m_pMyHuman->m_vecPosition.x;
	const int my = (int)m_pMyHuman->m_vecPosition.y;
	const int isInPos = (mx >> 7 == 8 || mx >> 7 == 9) && (my >> 7 == 15 || my >> 7 == 16);
	if (TMFieldScene::m_bPK == 1 && g_bCastleWar > 0 && m_pMyHuman->m_cMantua > 0 &&
		m_pMyHuman->m_cMantua == pOver->m_cMantua && !isInPos)
		return 0;
	if (!TMFieldScene::m_bPK && g_bCastleWar > 0 && m_pMyHuman->m_cMantua == 3 &&
		(IsTargetUser || pOver->m_cMantua > 0 && pOver->m_cMantua != 4))
		return 0;
	if (!TMFieldScene::m_bPK && g_bCastleWar > 0 && !pOver->IsInCastleZone() && IsTargetUser && !isInPos)
		return 0;

	// MoveAttack — portões extras do auto-ataque
	if ((m_pMyHuman->m_stScore.Merchant & 0xF) == 15 && m_pMyHuman->m_cMantua > 0 &&
		m_pMyHuman->m_cMantua == pOver->m_cMantua)
		return 0;
	if (mx >= 2362 && mx <= 2370 && my >= 3927 && my <= 3935)
		return 0;
	if (!TMFieldScene::m_bPK && pOver->m_cSummons == 1 && g_pObjectManager->m_usWarGuild == pOver->m_usGuild &&
		pOver->m_usGuild && g_bCastleWar)
	{
		const int outPos = mx >> 7 <= 16 || mx >> 7 >= 20 || my >> 7 <= 29;
		if (outPos || (m_pMyHuman->m_cMantua > 0 && pOver->m_cMantua == m_pMyHuman->m_cMantua))
			return 0;
	}
	if (!TMFieldScene::m_bPK)
	{
		if (!g_bCastleWar && (mx >> 7 <= 16 || mx >> 7 >= 20 || my >> 7 <= 29))
		{
			if (pOver->m_usGuild && g_pObjectManager->m_usWarGuild != pOver->m_usGuild)
				return 0;
			if (g_pObjectManager->m_usWarGuild > 0 && g_pObjectManager->m_usWarGuild != pOver->m_usGuild && IsTargetUser)
				return 0;
		}
		else if (m_pMyHuman->m_cMantua > 0 && pOver->m_cMantua == m_pMyHuman->m_cMantua)
			return 0;
	}

	// Laço nativo de m_cAutoAttack (FrameMove)
	if (m_pMyHuman->m_usGuild && pOver->m_usGuild &&
		(m_pMyHuman->m_usGuild == pOver->m_usGuild || g_pObjectManager->m_usAllyGuild == pOver->m_usGuild))
		return 0;
	return 1;
}
		...
		if (!IsTargetUser && !pOver->m_bParty && !pOver->m_cSummons || pOver->IsInPKZone())
		{
			...
			if (nDistance <= nMobAttackRange && nTX == (signed int)pOver->m_vecPosition.x && nTY == (signed int)pOver->m_vecPosition.y)
			{
				...
				if (pMobData->Class == 3 && pMobData->LearnedSkill[0] & 0x40)
				{
					...
					for (auto* pNode = (TMHuman*)m_pHumanContainer->m_pDown; pNode;
						pNode = (TMHuman*)pNode->m_pNextLink)
					{
						if (pNode == m_pMyHuman || pNode == pOver
							|| (int)pNode->m_vecPosition.x != TX || (int)pNode->m_vecPosition.y != TY)
							continue;
						...
						if (pNode->m_dwID > 0 && pNode->m_dwID < 1000)
						{
							...
							if (!TMFieldScene::m_bPK && g_pObjectManager->m_usWarGuild != pNode->m_usGuild && !g_bCastleWar)
								continue;
							if (!TMFieldScene::m_bPK && m_pMyHuman->m_cMantua > 0
								&& m_pMyHuman->m_cMantua == pNode->m_cMantua && g_bCastleWar > 0)
								continue;
							if (!TMFieldScene::m_bPK && g_bCastleWar > 0 && m_pMyHuman->m_cMantua == 3
								&& ((pNode->m_dwID > 0 && pNode->m_dwID < 1000)
									|| (pNode->m_cMantua > 0 && pNode->m_cMantua != 4)))
						...
						{
							if ((!TMFieldScene::m_bPK && pNode->m_cSummons == 1
									&& (g_pObjectManager->m_usWarGuild != pNode->m_usGuild && pNode->m_usGuild))
								|| !pNode->m_usGuild)
								continue;
						}
...
int TMFieldScene::TimeDelay(unsigned int dwServerTime)
{
	...
	if (m_dwLastTown)
	{
		...
		{
			// This countdown belongs to the town-return request, not the
			// select-server countdown. Using m_dwLastSelServer here leaves the
			// unsigned expression wrapped when the player returns from death
			// (m_dwLastSelServer is normally zero), creating a new portal effect
			// every frame until the client is killed by uncontrolled allocation.
			const unsigned int townDeadline = m_dwLastTown + 5000;
			const unsigned int dwRemain = townDeadline > dwServerTime
				? (townDeadline - dwServerTime) / 1000
				: 0;
			if (m_dwLastRemain != dwRemain)
			{
				m_bAutoRun = 0;
...
void TMFieldScene::SetVisibleCharInfo()
{
	...
	if (m_bCompatFieldScene)
	{
		if (m_pCPanel)
		{
			const int show = m_pCPanel->IsVisible() == 0;
			m_pCPanel->SetVisible(show);
#if defined(__SWITCH__)
			if (show)
			{
				PositionCompatFeaturePanels();
				// Alvos de toque maiores nos botões "+" de status/mastery.
				static const unsigned int kPlusIds[] = {
					1073u, 1076u, 1079u, 1088u, 1139u, 1142u, 1145u, 1154u,
					65716u, 65719u, 65722u, 65725u, 65754u, 65757u, 65760u, 65763u
				};
				const float minW = 36.0f * RenderDevice::m_fWidthRatio;
				const float minH = 36.0f * RenderDevice::m_fHeightRatio;
				for (unsigned int id : kPlusIds)
				{
					if (auto* btn = static_cast<SButton*>(m_pControlContainer->FindControl(id)))
					{
						const float w = btn->m_nWidth < minW ? minW : btn->m_nWidth;
						const float h = btn->m_nHeight < minH ? minH : btn->m_nHeight;
						btn->SetSize(w, h);
						btn->m_bSelectEnable = 1;
						btn->m_bEnable = 1;
					}
				}
			}
...
void TMFieldScene::SetVisibleSkillMaster()
{
	...
	if (bVisible == 1)
	{
		...
		// Refresh remaining SkillPts + learned icons whenever the apprentice
		// pair opens; UpdateEtc alone is not enough if the panel was closed.
		UpdateScoreUI(0);
...
void TMFieldScene::UpdateScoreUI(unsigned int unFlag)
{
	...
	if (!(unFlag & 0x10))
	{
		sprintf(szStr, WYD_FMT_I64, pMobData->Exp);
		if (m_pCIEXP)
		{
			m_pCIEXP->m_cComma = 1;
		...
		if (ckind == 2)
			sprintf(szStr, WYD_FMT_I64, g_pNextLevel_G2[CurLevel + 1]);
		else
			sprintf(szStr, WYD_FMT_I64, g_pNextLevel[CurLevel + 1]);
...
{
	auto pCamera = g_pObjectManager ? g_pObjectManager->m_pCamera : nullptr;
	if (!pCamera || !g_pDevice)
		return;
	...
	auto* focused = pCamera->GetFocusedObject();
	if (m_nCameraMode == 0)
	{
		if (!focused)
			return;
		TMVector2 Pos = focused->m_vecPosition;
	...
	auto pBtnCamera = m_pControlContainer
		? (SButton*)m_pControlContainer->FindControl(308)
		: nullptr;
	if (pBtnCamera)
...
{
	auto pMobData = &g_pObjectManager->m_stMobData;
	const int idxL = pMobData->Equip[6].sIndex;
	const int idxR = pMobData->Equip[7].sIndex;
	if (idxL < 0 || idxL >= MAX_ITEMLIST || idxR < 0 || idxR >= MAX_ITEMLIST)
		return 0;

	int w1 = BASE_GetItemAbility(&g_pObjectManager->m_stMobData.Equip[6], 2);
	int w2 = BASE_GetItemAbility(&pMobData->Equip[7], 2);
	int t1 = g_pItemList[idxL].nUnique;
	int t2 = g_pItemList[idxR].nUnique;
...
void TMFieldScene::SetWeather(int nWeather)
{
	...
	{
	case 1:
		if (m_pRain) m_pRain->m_bVisible = 1;
		if (m_pSnow) m_pSnow->m_bVisible = 0;
		if (m_pSnow2) m_pSnow2->m_bVisible = 0;
		if (m_pSky) m_pSky->SetWeatherState(11);
		break;
	case 2:
		if (m_pRain) m_pRain->m_bVisible = 0;
		if (m_pSnow) m_pSnow->m_bVisible = 1;
		if (m_pSnow2) m_pSnow2->m_bVisible = 0;
		if (m_pSky) m_pSky->SetWeatherState(11);
		break;
	case 3:
		if (m_pRain) m_pRain->m_bVisible = 0;
		if (m_pSnow) m_pSnow->m_bVisible = 1;
		if (m_pSnow2) m_pSnow2->m_bVisible = 1;
		if (m_pSky) m_pSky->SetWeatherState(11);
		break;
	default:
		// Clear weather (0): previous switch left sky/fog untouched, so the
		// field kept SelectServer's black m_dwClearColor on Linux/DXVK.
		if (m_pRain) m_pRain->m_bVisible = 0;
		if (m_pSnow) m_pSnow->m_bVisible = 0;
		if (m_pSnow2) m_pSnow2->m_bVisible = 0;
		if (m_pSky) m_pSky->SetWeatherState(0);
		break;
	}
}
...
extern "C" int WYD_Linux_MacroState(void);
extern "C" int WYD_Linux_PKState(void);

namespace
{
constexpr unsigned int kSwitchCtlTabId = 990801u;
constexpr unsigned int kSwitchCtlListId = 990802u;
constexpr unsigned int kSwitchCtlUpId = 990803u;
constexpr unsigned int kSwitchCtlDownId = 990804u;
constexpr unsigned int kSwitchMacroTabId = 990805u;
}

void TMFieldScene::SwitchControlsTabInit()
{
	if (m_pSwitchCtlTab || !m_pHelpPanel || !m_pHelpButton[3] || !m_pHelpList[1])
		return;
	auto* listener = static_cast<IEventListener*>(m_pControlContainer);

	SButton* lastTab = m_pHelpButton[3];
	TreeNode* tabParent = lastTab->m_pTop ? lastTab->m_pTop : m_pHelpPanel;
	const float tabW = lastTab->m_nWidth;
	const float tabH = lastTab->m_nHeight;
	float tabY = lastTab->m_nPosY;
	float tabX = lastTab->m_nPosX + tabW + 2.0f;
	const float screenW = g_pDevice
		? static_cast<float>(g_pDevice->m_dwScreenWidth)
		: 1280.0f;
	constexpr float kTabMargin = 6.0f;
	// CONTROLE + MACRO: se não cabem à direita da 4ª aba, sobem para a linha de baixo.
	if (tabX + tabW * 2.0f + 2.0f > screenW - kTabMargin)
	{
		tabX = lastTab->m_nPosX;
		tabY = lastTab->m_nPosY + tabH + 2.0f;
		if (tabX + tabW * 2.0f + 2.0f > screenW - kTabMargin)
			tabX = std::max(kTabMargin, screenW - kTabMargin - tabW * 2.0f - 2.0f);
	}

	m_pSwitchCtlTab = new SButton(14, tabX, tabY, tabW, tabH, 0, 1, const_cast<char*>("CONTROLE"));
	m_pSwitchCtlTab->SetControlID(kSwitchCtlTabId);
	m_pSwitchCtlTab->SetEventListener(listener);
	tabParent->AddChild(m_pSwitchCtlTab);

	tabX += tabW + 2.0f;
	m_pSwitchMacroTab = new SButton(14, tabX, tabY, tabW, tabH, 0, 1, const_cast<char*>("MACRO"));
	m_pSwitchMacroTab->SetControlID(kSwitchMacroTabId);
	m_pSwitchMacroTab->SetEventListener(listener);
	tabParent->AddChild(m_pSwitchMacroTab);

	SListBox* ref = m_pHelpList[1];
	TreeNode* listParent = ref->m_pTop ? ref->m_pTop : m_pHelpPanel;
	const float btnW = 30.0f;
	const float listW = ref->m_nWidth - btnW - 4.0f;
	int visible = static_cast<int>(ref->m_nHeight / (16.0f * RenderDevice::m_fHeightRatio));
	if (visible < 4)
		visible = 4;
	m_pSwitchCtlList = new SListBox(-2, 64, visible, ref->m_nPosX, ref->m_nPosY, listW, ref->m_nHeight,
		0xC0101418u, RENDERCTRLTYPE::RENDER_IMAGE_STRETCH, 1, 0, 0);
	m_pSwitchCtlList->SetControlID(kSwitchCtlListId);
	m_pSwitchCtlList->SetEventListener(listener);
	listParent->AddChild(m_pSwitchCtlList);

	const float bx = ref->m_nPosX + listW + 4.0f;
	m_pSwitchCtlUp = new SButton(14, bx, ref->m_nPosY, btnW, 24.0f, 0, 1, const_cast<char*>("/\\"));
	m_pSwitchCtlDown = new SButton(14, bx, ref->m_nPosY + ref->m_nHeight - 24.0f, btnW, 24.0f, 0, 1,
		const_cast<char*>("\\/"));
	m_pSwitchCtlUp->SetControlID(kSwitchCtlUpId);
	m_pSwitchCtlDown->SetControlID(kSwitchCtlDownId);
	m_pSwitchCtlUp->SetEventListener(listener);
	m_pSwitchCtlDown->SetEventListener(listener);
	listParent->AddChild(m_pSwitchCtlUp);
	listParent->AddChild(m_pSwitchCtlDown);

	// Indicador persistente abaixo do painel de HP (canto superior esquerdo).
	m_pSwitchMacroHud = new SText(-2, const_cast<char*>("MACRO OFF | PK OFF"), 0xFFCCDDDDu,
		10.0f * RenderDevice::m_fWidthRatio, 160.0f * RenderDevice::m_fHeightRatio,
		280.0f * RenderDevice::m_fWidthRatio, 18.0f * RenderDevice::m_fHeightRatio,
		1, 0xAA000000u, 1, 0);
	m_pSwitchMacroHud->SetControlID(990806u);
	m_pControlContainer->AddItem(static_cast<SControl*>(m_pSwitchMacroHud));
	m_pSwitchMacroHud->SetVisible(0);

	SwitchControlsTabShow(0, 0);
}

void TMFieldScene::SwitchControlsTabShow(int bShow, int page)
{
	if (!m_pSwitchCtlList)
		return;
	if (bShow)
		WYD_Switch_ControlsSetPage(page);
	m_pSwitchCtlList->SetVisible(bShow);
	m_pSwitchCtlUp->SetVisible(bShow);
	m_pSwitchCtlDown->SetVisible(bShow);
	if (m_pSwitchCtlTab)
		m_pSwitchCtlTab->SetSelected(bShow && page == 0);
	if (m_pSwitchMacroTab)
		m_pSwitchMacroTab->SetSelected(bShow && page == 1);
	m_nSwitchCtlPage = bShow ? page : -1;
	if (bShow)
	{
		if (m_pHelpInterface)
			m_pHelpInterface->SetVisible(0);
	...
	else if (m_pHelpList[0])
		m_pHelpList[0]->SetVisible(1);
}

void TMFieldScene::SwitchOpenMacroTab()
{
	if (!m_pHelpPanel)
		return;
	if (!m_pHelpPanel->IsVisible())
		m_pHelpPanel->SetVisible(1);
	SwitchControlsTabShow(1, 1);
}

void TMFieldScene::SwitchControlsTabTick()
{
	if (WYD_Switch_ConsumeOpenMacro())
		SwitchOpenMacroTab();

	if (m_pSwitchMacroHud && m_pSwitchMacroHud->IsVisible())
		m_pSwitchMacroHud->SetVisible(0);

	if (!m_pSwitchCtlList)
		return;
	const int active = m_pSwitchCtlList->IsVisible() && m_pHelpPanel && m_pHelpPanel->IsVisible();
	WYD_Switch_ControlsSetActive(active);
	if (active)
	{
		const int page = WYD_Switch_ControlsPage();
		if (m_nSwitchCtlPage != page)
		{
			m_nSwitchCtlPage = page;
			if (m_pSwitchCtlTab)
				m_pSwitchCtlTab->SetSelected(page == 0);
			if (m_pSwitchMacroTab)
				m_pSwitchMacroTab->SetSelected(page == 1);
			m_dwSwitchCtlRev = 0;
		}
	}
	if (!active || m_dwSwitchCtlRev == WYD_Switch_ControlsRevision())
		return;
	m_dwSwitchCtlRev = WYD_Switch_ControlsRevision();

	const int start = m_pSwitchCtlList->m_nStartItemIndex;
	m_pSwitchCtlList->Empty();
	int selected = -1;
	const int count = WYD_Switch_ControlsLineCount();
	for (int i = 0; i < count; ++i)
	{
		unsigned int color = 0xFFFFFFFFu;
		const char* text = WYD_Switch_ControlsLine(i, &color);
		if (text[0] == '>')
			selected = i;
		m_pSwitchCtlList->AddItem(new SListBoxItem(text, color, 0.0f, 0.0f, m_pSwitchCtlList->m_nWidth, 16.0f,
			0, 0x77777777u, 1u, 0));
	}
	const int vis = m_pSwitchCtlList->m_nVisibleCount;
	int first = start;
	if (selected >= 0 && selected < first)
		first = selected;
	if (selected >= 0 && selected >= first + vis)
		first = selected - vis + 1;
	if (first > count - vis)
		first = count - vis;
	if (first < 0)
		first = 0;
	m_pSwitchCtlList->SetStartItemIndex(first);
}

int TMFieldScene::SwitchControlsTabEvent(unsigned int idwControlID, unsigned int idwEvent)
{
	if (!m_pSwitchCtlList)
		return 0;
	switch (idwControlID)
	{
	case kSwitchCtlTabId:
		SwitchControlsTabShow(1, 0);
		return 1;
	case kSwitchMacroTabId:
		SwitchControlsTabShow(1, 1);
		return 1;
	case kSwitchCtlListId:
		WYD_Switch_ControlsClick(static_cast<int>(idwEvent));
		return 1;
	case kSwitchCtlUpId:
	case kSwitchCtlDownId:
	{
		const int step = m_pSwitchCtlList->m_nVisibleCount - 1;
		int first = m_pSwitchCtlList->m_nStartItemIndex + (idwControlID == kSwitchCtlUpId ? -step : step);
		const int maxFirst = m_pSwitchCtlList->m_nNumItem - m_pSwitchCtlList->m_nVisibleCount;
		if (first > maxFirst)
			first = maxFirst;
		if (first < 0)
			first = 0;
	...
	case TMB_HELP_BUTTON1:
	case TMB_HELP_BUTTON2:
	case TMB_HELP_BUTTON3:
	case TMB_HELP_BUTTON4:
		if (m_pSwitchCtlList->IsVisible())
			SwitchControlsTabShow(0, 0);
		return 0;
	default:
		return 0;
...
char TMFieldScene::UseQuickSloat(char key)
{
	...
	// Compat FieldScene2 often lacks the modern quick grids; never deref null.
	if (slot < 0 || slot >= 5 || m_pQuick_Sloat[slot] == nullptr)
		return 0;

	pGridSloat = m_pQuick_Sloat[slot];
	pItemFind = pGridSloat->GetAtItem(0, 0);
	...
	int SourPos = -1;
	if (pItem->m_pGridControl)
		SourPos = pGridInv->CheckPos(pItem->m_pGridControl->m_eItemType);
	if (SourPos == -1)
		SourPos = pItem->m_nCellIndexX + 9 * pItem->m_nCellIndexY;
	if (SourPos < 0 || SourPos >= MAX_CARRY - 1)
		return 0;
...
int TMFieldScene::OnMsgBoxEvent(unsigned int idwControlID, unsigned int idwEvent, unsigned int dwServerTime)
{
	...
	case 4:
	{
		...
		// Detail = skill book item index; TargetID = skill-master NPC id.
		// Keep the packed MessageBox arg split explicit so high bits never
		// leak into TargetID (unsigned short truncation alone is easy to miss).
		stApplyBonus.Detail = static_cast<short>(m_pMessageBox->m_dwArg >> 16);
		stApplyBonus.TargetID = static_cast<unsigned short>(m_pMessageBox->m_dwArg & 0xFFFFu);
		SendPacket({reinterpret_cast<MSG_STANDARD*>(&stApplyBonus)->Type, reinterpret_cast<char*>(&stApplyBonus), sizeof(stApplyBonus)});
	}
	break;
	case 6193:
...
#if defined(__SWITCH__)
		// Ao abrir a ajuda no Switch, ir direto à aba CONTROLE (remap) e
		// esconder as listas de texto nativas que sobrepunham o menu.
		if (bVisible == 0)
			SwitchControlsTabShow(1, 0);
		else
			SwitchControlsTabShow(0, 0);
...
int TMFieldScene::OnPacketCreateMobCompat(MSG_STANDARD* pStd)
{
	...
	const unsigned short pktSize = pStd->Size;
	if (pStd->Type == MSG_CreateMobTrade_Opcode)
	{
		if (pktSize < sizeof(MSG_CreateMobTrade))
			return 1;
	}
	else if (pktSize < sizeof(MSG_CreateMob))
	{
		return 1;
	}

	MSG_CreateMob createMob{};
	MSG_CreateMobTrade createTrade{};
	const bool isTrade = (pStd->Type == MSG_CreateMobTrade_Opcode);
	if (isTrade)
	{
		std::memcpy(&createTrade, pStd, sizeof(MSG_CreateMobTrade));
		// Layout até Nick coincide; Server do trade fica após Desc — copiar campos
		// usados pelo spawn para createMob e aplicar override de loja como o path full.
		std::memcpy(&createMob, &createTrade, sizeof(MSG_CreateMob));
		createMob.Server = createTrade.Server;
	}
	else
	{
		std::memcpy(&createMob, pStd, sizeof(MSG_CreateMob));
	}
	auto* pCreateMob = &createMob;
	WYD748_DiagnosticsLog("compat CreateMob id=%u at (%d,%d) equip0=%u trade=%d\r\n",
		pCreateMob->MobID, pCreateMob->PosX, pCreateMob->PosY,
		pCreateMob->Equip[0] & 0x0FFF, isTrade ? 1 : 0);

	// Paridade CreateMobTrade: loja com descrição usa corpo genérico 230.
	if (isTrade && createTrade.Desc[0])
	{
	...
	auto* pExisting = static_cast<TMHuman*>(g_pObjectManager->GetHumanByID(pCreateMob->MobID));
	// Revive só se morto / DelayDelete. Rematerializar vivo só quando o head
	// ficou no default 0 (TK preso) — refresh em todo head diverge destruía
	// skins e podia sumir com o personagem (Init + ItemList errado).
	const bool needRevive = pExisting && (pExisting->m_nWillDie != -1 || pExisting->m_cDie == 1
		|| pExisting->m_dwDelayDel != 0);
	const unsigned short eq0 = static_cast<unsigned short>(pCreateMob->Equip[0] & 0x0FFF);
	const bool stuckDefaultHead = pExisting && !needRevive
		&& pExisting->m_sHeadIndex == 0 && eq0 != 0;
	if (pExisting && !needRevive && !stuckDefaultHead)
		return 1;
	if (stuckDefaultHead)
	{
		WYD748_DiagnosticsLog("compat CreateMob REFRESH stuckHead id=%u newEquip0=%u\r\n",
			pCreateMob->MobID, eq0);
	}

	TMHuman* pHuman = pExisting;
	if (!pHuman)
	{
	...
	{
		const unsigned int dwOldMoveTime = pHuman->m_dwOldMovePacketTime;
		pHuman->m_nWillDie = -1;
		pHuman->m_dwDeadTime = 0;
		pHuman->m_dwDelayDel = 0;
		pHuman->Init();
		pHuman->m_dwOldMovePacketTime = dwOldMoveTime;
		WYD748_DiagnosticsLog("compat CreateMob REVIVE/REFRESH id=%u\r\n", pCreateMob->MobID);
	}

	// CreateMob autoritativo: permitir SetPacketEquipItem/SetRace.
	pHuman->m_dwDelayDel = 0;
	...
	// Jogadores (MobID < 1000): chaos/kills embutidos no MobName[12..14], como o path full.
	if (pCreateMob->MobID > 0 && pCreateMob->MobID < 1000)
	{
		const char cCurrent = pCreateMob->MobName[13];
		const short* sTotal = reinterpret_cast<short*>(&pCreateMob->MobName[14]);
		pHuman->m_nCurrentKill = static_cast<unsigned char>(cCurrent);
		pHuman->m_nTotalKill = *sTotal;
		pHuman->m_ucChaosLevel = static_cast<unsigned char>(pCreateMob->MobName[12]);
	...
	// Montaria: ID completo (Equip[14]) tem prioridade sobre o byte Equipe2.
	...
	{
		pHuman->SetMountCostume(static_cast<unsigned char>(pCreateMob->Equip2[14]));
	}
	...
	const float fCon = static_cast<float>(pHuman->m_stScore.Con);
	pHuman->SetCharHeight(fCon);
	// Match the full CreateMob path: human faces (<40) get a slight shrink so
	// mob Con-driven scale does not read as "giants" next to the player.
	if ((pCreateMob->Equip[0] & 0x0FFF) < 40)
		pHuman->m_fScale *= 0.89999998f;
	pHuman->SetRace(pCreateMob->Equip[0] & 0x0FFF);

	// Arma tipo 41: espelha mesh/skin esquerda → direita (path full).
	{
		STRUCT_ITEM itemL{};
		itemL.sIndex = static_cast<short>(pCreateMob->Equip[6] & 0x0FFF);
		if (BASE_GetItemAbility(&itemL, 21) == 41)
		{
	...
	WYD748_DiagnosticsLog(
		"compat CreateMob skin id=%u equip0=%u head=%d class=%d meshType=%d Con=%d scale=%.3f\r\n",
		pCreateMob->MobID, pCreateMob->Equip[0] & 0x0FFF,
		pHuman->m_sHeadIndex, pHuman->m_nClass, pHuman->m_nSkinMeshType,
		pHuman->m_stScore.Con, pHuman->m_fScale);
	pHuman->InitObject();
	if (isTrade)
	{
		...
		memcpy(pHuman->m_TradeDesc, createTrade.Desc, sizeof(pHuman->m_TradeDesc));
	...
	// Guild mark (mesmo bloco do CreateMob full).
	pHuman->m_sGuildLevel = static_cast<unsigned char>(pCreateMob->GuildLevel);
	const unsigned short usGuild = pCreateMob->Guild;
	if (usGuild)
	{
		const int nSubGuild = BASE_GetSubGuild(pHuman->m_sGuildLevel);
		pHuman->m_stGuildMark.nGuild = usGuild & 0xFFF;
		pHuman->m_stGuildMark.nSubGuild = nSubGuild;
		pHuman->m_stGuildMark.nGuildChannel = (static_cast<int>(usGuild) >> 12) & 0xF;
		switch (pHuman->m_sGuildLevel)
		{
		case 0: pHuman->m_stGuildMark.sGuildIndex = 508; break;
		case 1: pHuman->m_stGuildMark.sGuildIndex = 535; break;
		case 2: pHuman->m_stGuildMark.sGuildIndex = 508; break;
		case 3: pHuman->m_stGuildMark.sGuildIndex = 526; break;
		case 4: pHuman->m_stGuildMark.sGuildIndex = 527; break;
		case 5: pHuman->m_stGuildMark.sGuildIndex = 528; break;
		case 6: pHuman->m_stGuildMark.sGuildIndex = 529; break;
		case 7: pHuman->m_stGuildMark.sGuildIndex = 530; break;
		case 8: pHuman->m_stGuildMark.sGuildIndex = 531; break;
		case 9: pHuman->m_stGuildMark.sGuildIndex = 509; break;
		default: break;
		}
		if (pStd->Type != MSG_CreateMobTrade_Opcode
			&& pHuman->m_pAutoTradeDesc && !pHuman->m_pAutoTradeDesc->IsVisible())
	...
	// Altura via GroundGetMask (igual path full) — GetHeight diverge em rampas.
...
{
	if (m_pHumanContainer == nullptr || pStd == nullptr)
		return 1;
	...
	// Copiar para structs alinhadas: o buffer de recv pode estar desalinhado (ARM64).
	MSG_CreateMob createMobLocal{};
	std::memcpy(&createMobLocal, pStd, sizeof(MSG_CreateMob));
	MSG_CreateMobTrade createTradeLocal{};
	if (pStd->Type == MSG_CreateMobTrade_Opcode)
		std::memcpy(&createTradeLocal, pStd, sizeof(MSG_CreateMobTrade));

	auto* pCreateMob = &createMobLocal;
	auto* pCreateMobTrade = &createTradeLocal;
...
{
	if (!pStd || !g_pObjectManager || pStd->ID != g_pObjectManager->m_dwCharID)
		return 1;

	if (g_pDevice)
		g_pDevice->m_nWidthShift = 0;

	// A map/terrain recovery can race the logout packet before the local human
	// has been materialized. The old handler dereferenced both pointers and
	// crashed instead of returning cleanly to character selection.
	const int nSlot = g_pObjectManager->m_cCharacterSlot;
	if (m_pMyHuman && nSlot >= 0 && nSlot < 4)
	{
		auto* pSelChar = &g_pObjectManager->m_stSelCharData;
		memcpy(&pSelChar->Score[nSlot], &m_pMyHuman->m_stScore, sizeof(m_pMyHuman->m_stScore));
		memcpy(pSelChar->Equip[nSlot], g_pObjectManager->m_stMobData.Equip,
			sizeof(g_pObjectManager->m_stMobData.Equip));
	}

	// CNFCharacterLogout is a character-session transition, not a socket
	// disconnect. Keep the authenticated connection alive for the next slot.
...
int TMFieldScene::OnPacketAddParty(MSG_AddParty* pStd)
{
	...
	// Party roster updates must not reopen a panel the player intentionally
	// closed; the player controls visibility through the party button/menu.
		...
		g_pSpell[pAttack->SkillIndex].TargetType != 5 && g_pSpell[pAttack->SkillIndex].TargetType != 6)
	{
		for (int i = 0; i < 13; i++)
		{
			...
			if (pTargetHuman)
			{
				...
				// Feedback de dano em tempo real: sem sincronizar com o frame
				// da animação de ataque (antes: 0–1000 ms via dwDelayTable).
				...
				if (pAttacker != m_pMyHuman || !pAttack->FlagLocal && pAttacker == m_pMyHuman)
				{
					...
					pTargetHuman->m_stPunchEvent.dwTime = g_pTimerManager->GetServerTime();
				}
			}
		}
	}
...
void TMFieldScene::Bag_View()
{
	...
	// bag-page controls added by the 7.59 UI.  CreateMob / UpdateScore still call
	// Bag_View for the local player.  Always require the controls — do not gate
	// on m_bCompatFieldScene alone: an uninitialized non-null garbage pointer
	// used to pass the old check and SIGSEGV at SetTextureSetIndex.
	if (!m_pInvPageBtn3 || !m_pInvPageBtn4 || !m_pJPNBag_Day1 || !m_pJPNBag_Day2)
		return;
...
int TMFieldScene::Affect_Main(unsigned int dwServerTime)
{
	...
	if (m_bCompatFieldScene)
	{
		...
		for (int affectIndex = 0; affectIndex < 16; ++affectIndex)
		{
			...
			SText* pTime = m_pAffect[affectIndex];
			if (!pIcon)
				continue;
			...
			if (pTime)
			{
				...
				pTime->SetText(timeText, 0);
				pTime->SetTextColor(secondsLeft <= 10 ? 0xFFFF5555u : 0xFFFFFFFFu);
				pTime->SetPos(pIcon->m_nPosX - 4.0f, pIcon->m_nPosY + 24.0f);
				pTime->SetVisible(secondsLeft > 0 ? 1 : 0);
			}
	...
	for (int i = 0; i < 32; ++i)
	{
		...
		if ((unsigned char)m_pMyHuman->m_stAffect[i].Type <= 50)
		{
			if (!m_pAffectIcon[i]->IsVisible() && m_pAffect[i])
...
void TMFieldScene::SetClickTargetAtPick(const D3DXVECTOR3& pick)
{
	if (!m_pTarget1 || !m_pTarget2 || !m_pTargetBill)
		return;

	const float fHeight = static_cast<float>(GroundGetMask(TMVector2{ pick.x, pick.z })) * 0.1f;
	const TMVector3 vecTar{ pick.x, fHeight + 0.30000001f, pick.z };
	m_pTarget1->m_vecPosition = vecTar;
	m_pTarget2->m_vecPosition = vecTar;
	m_pTargetBill->m_vecPosition = vecTar;
	m_pTargetBill->m_vecPosition.y -= 0.2f;
...
void TMFieldScene::IssueMoveToPick(const D3DXVECTOR3& pick, bool holdRepeat)
{
	if (!m_pMyHuman)
		return;
	if (pick.y < -5000.0f)
	...
	const unsigned int now = g_pTimerManager->GetServerTime();
	if (!m_pMyHuman->CanCancelCombatWithMove(now))
		return;

	const int cellX = static_cast<int>(pick.x);
	const int cellY = static_cast<int>(pick.z);
	// Hold-to-move repaths every poll would chase a moving ground pick (camera
	// follows the character), which snaps facing/route and feels like the
	// camera resets toward the click. Only re-issue when the cell changes and
	// after a short throttle.
	if (holdRepeat)
	{
		if (cellX == m_vecLastIssueMoveCell.x && cellY == m_vecLastIssueMoveCell.y)
			return;
		if (now - m_dwLastIssueMoveTime < 150)
			return;
	}

	m_vecLastIssueMoveCell.x = cellX;
	m_vecLastIssueMoveCell.y = cellY;
	m_dwLastIssueMoveTime = now;

	SetClickTargetAtPick(pick);

	m_pMyHuman->SetSpeed(m_bMountDead);
	m_vecMyNext.x = cellX;
	m_vecMyNext.y = cellY;
	// GetRoute already respects packet delay and redraws from progress >0.5.
	m_pMyHuman->GetRoute(m_vecMyNext, 0, 0);
...
// Movimento por analógico: passo curto em linha reta na direção do stick, sem
// marcador de clique. O alvo é cortado no primeiro obstáculo (mesmo teste de
// linha do MoveAttack), então o personagem não contorna paredes nem "escorrega"
// para rotas longas em pontes, portas e teleportes.
void TMFieldScene::IssueStickStep(float dirX, float dirY, bool stop)
{
	if (!m_pMyHuman || m_pMyHuman->m_cCantMove || !g_pTimerManager)
		return;
	const unsigned int now = g_pTimerManager->GetServerTime();
	if (!m_pMyHuman->CanCancelCombatWithMove(now))
		return;

	const float px = m_pMyHuman->m_vecPosition.x;
	const float py = m_pMyHuman->m_vecPosition.y;
	...
	if (!stop)
	{
		// GetRoute só aceita rota nova após este intervalo (fora o stop); o passo
		// precisa cobrir a distância andada nele para não parar e voltar a andar.
		const int speed = static_cast<int>(m_pMyHuman->m_fMaxSpeed);
		const float delay = (speed == 6 || speed == 7) ? 0.1f : (speed == 4 || speed == 5) ? 0.5f : 1.0f;
		look = static_cast<float>(speed) * delay + 1.0f;
		if (look < 2.0f)
			look = 2.0f;
		if (look > 5.0f)
			look = 5.0f;
	...
	int tx = static_cast<int>(px + dirX * look);
	int ty = static_cast<int>(py + dirY * look);
	if (!stop)
	{
		BASE_GetHitPosition(sx, sy, &tx, &ty, reinterpret_cast<char*>(m_HeightMapData), 8);
		if ((!tx && !ty) || (tx == sx && ty == sy))
			return;
		if (tx == m_vecLastIssueMoveCell.x && ty == m_vecLastIssueMoveCell.y && now - m_dwLastIssueMoveTime < 150)
			return;
	}

	m_vecLastIssueMoveCell.x = tx;
	m_vecLastIssueMoveCell.y = ty;
	m_dwLastIssueMoveTime = now;

	m_pMyHuman->SetSpeed(m_bMountDead);
	m_vecMyNext.x = tx;
	m_vecMyNext.y = ty;
	m_pMyHuman->GetRoute(m_vecMyNext, 0, stop ? 1 : 0);
...
void TMFieldScene::UpdateClickTargetVisuals(unsigned int dwServerTime)
{
	if (!m_pMyHuman || !m_pTarget1 || !m_pTarget2 || !m_pTargetBill)
		return;
	...
	const bool walking =
		(m_pMyHuman->m_eMotion == ECHAR_MOTION::ECMOTION_WALK ||
			m_pMyHuman->m_eMotion == ECHAR_MOTION::ECMOTION_RUN)
		&& !m_bAutoRun
		&& !m_pMyHuman->m_pMoveTargetHuman
		&& !m_pMyHuman->m_pMoveSkillTargetHuman
		&& !m_pTargetItem;

	m_pTarget1->m_bShow = walking ? 1 : 0;
	m_pTarget2->m_bShow = walking ? 1 : 0;
	m_pTargetBill->m_bShow = walking ? 1 : 0;
}

void TMFieldScene::UpdateGroundTeleportPrompt(unsigned int dwServerTime)
{
	(void)dwServerTime;
	if (!m_pMyHuman || !m_pMessageBox)
		return;

	const int px = (int)m_pMyHuman->m_vecPosition.x;
	const int py = (int)m_pMyHuman->m_vecPosition.y;
	const auto bAttr = BASE_GetAttr(px, py);
	const int x = px & 0xFFFC;
	const int y = py & 0xFFFC;

	// Aceita o ponto exato da tabela e a célula 4x4 vizinha (pads reais nem
	// sempre caem no & 0xFFFC idêntico ao registro).
	int nPosIndex = -1;
	int bestDist = 5;
	for (int ll = 0; ll < _countof(g_TeleportTable); ++ll)
	{
		const int dx = abs(g_TeleportTable[ll].nX - x);
		const int dy = abs(g_TeleportTable[ll].nY - y);
		const int d = dx + dy;
		if (d < bestDist)
		{
			bestDist = d;
	...
	const bool onPad = (bAttr & 0x10) != 0;
	if (!onPad)
	{
		// Sem reset ao sair, o flag travava e o próximo pad nunca reabria a caixa.
		m_bTeleportMsg = 0;
		if (m_pMessageBox->IsVisible() == 1 && m_pMessageBox->m_dwMessage == 16)
			m_pMessageBox->SetVisible(0);
		return;
	}

	// Em cidade o personagem troca STAND por YAWN/RELAX/etc.; exigir só STAND01/02
	// fazia o prompt nunca aparecer. Bloqueia apenas enquanto está andando.
	const auto motion = m_pMyHuman->m_eMotion;
	if (motion == ECHAR_MOTION::ECMOTION_WALK || motion == ECHAR_MOTION::ECMOTION_RUN)
		return;
	...
	if ((px > 1963 && px < 1970 && py > 1770 && py < 1777))
	{
		m_bTeleportMsg = 1;
		if (m_pMessagePanel)
		{
	...
	if (px >= 2370 && px <= 2411 && py >= 1728 && py <= 1759)
	{
		m_bTeleportMsg = 1;
		if (m_pMessagePanel)
		{
	...
	char* pMsgPrimary = g_pMessageStringTable[208];
	char* pMsgPrice = nullptr;
	char szGoto[256]{};
	char szPrice[256]{};

	if (nPosIndex >= 0 && pMsgPrimary && pMsgPrimary[0])
	{
		sprintf(szGoto, pMsgPrimary, g_TeleportTable[nPosIndex].szTarget ? g_TeleportTable[nPosIndex].szTarget : "");
		pMsgPrimary = szGoto;
		...
		if (g_TeleportTable[nPosIndex].nPrice > 0 && nShowPrice == 1 &&
			g_pMessageStringTable[207] && g_pMessageStringTable[207][0])
		{
			sprintf(szPrice, g_pMessageStringTable[207], g_TeleportTable[nPosIndex].nPrice);
			pMsgPrice = szPrice;
		}
	}
	else if (!pMsgPrimary || !pMsgPrimary[0])
	{
		pMsgPrimary = const_cast<char*>("Deseja teleportar?");
	}

	m_pMessageBox->SetMessage(pMsgPrimary, 16, pMsgPrice);
	m_pMessageBox->SetVisible(1);
	m_bTeleportMsg = 1;
	printf("[teleport] prompt pad attr=0x%02x pos=(%d,%d) table=%d\n",
		(unsigned char)bAttr, px, py, nPosIndex);
}

void TMFieldScene::UpdateStatusPointHoldRepeat(unsigned int dwServerTime)
{
	auto pCPanel = m_pCPanel;
	if (!pCPanel || pCPanel->IsVisible() != 1 || !m_pControlContainer)
		return;

	int btPressType = 0;
	auto pickPlus = [&](unsigned int id) {
		auto* btn = (SButton*)m_pControlContainer->FindControl(id);
		if (btn && btn->IsOver() == 1)
			btPressType = static_cast<int>(id);
	};
	// Compat 7.48 (IDs nativos) e mapa 657xx.
	pickPlus(1073); pickPlus(65716);
	pickPlus(1076); pickPlus(65719);
	pickPlus(1079); pickPlus(65722);
	pickPlus(1088); pickPlus(65725);
	pickPlus(1139); pickPlus(65754);
	pickPlus(1142); pickPlus(65757);
	pickPlus(1145); pickPlus(65760);
	pickPlus(1154); pickPlus(65763);

	const bool holding = btPressType && g_pEventTranslator && g_pEventTranslator->button[0];
	if (!holding)
	{
		m_bClbutton = 0;
		m_dwLastClbuttonTime = 0;
	...
	if (!m_dwClbuttonHoldStart)
	{
		m_dwClbuttonHoldStart = dwServerTime;
		m_dwLastClbuttonTime = dwServerTime;
		m_bClbutton = 0;
		return;
	}

	const unsigned int heldMs = dwServerTime - m_dwClbuttonHoldStart;
	if (heldMs < 2000)
		return;
	...
	if (dwServerTime - m_dwLastClbuttonTime >= interval)
	{
		for (int n = 0; n < burst; ++n)
		{
			if (!OnControlEvent(static_cast<unsigned int>(btPressType), 0))
				break;
		}
		m_dwLastClbuttonTime = dwServerTime;
	}
...
void TMFieldScene::GameAuto()
{
	...
	// Só bloqueia animação de movimento/ataque; STAND01/02 (0/1) e NONE (-1) seguem.
	const auto motion = m_pMyHuman->m_eMotion;
	if (motion == ECHAR_MOTION::ECMOTION_WALK || motion == ECHAR_MOTION::ECMOTION_RUN ||
		(static_cast<int>(motion) >= static_cast<int>(ECHAR_MOTION::ECMOTION_ATTACK01) &&
		 static_cast<int>(motion) <= static_cast<int>(ECHAR_MOTION::ECMOTION_STRIKE)))
		return;
	...
	// Standing still with a stale LastSendTargetPos blocks every mode-0 swing.
	if (m_pMyHuman->m_fProgressRate <= 0.01f)
	{
		m_pMyHuman->m_LastSendTargetPos.x = nSX;
		m_pMyHuman->m_LastSendTargetPos.y = nSY;
	}

	// Tree siblings end with m_pNextLink == nullptr. The decompiled
	// `while (node && node->m_pNextLink)` skipped the last human (often the
	// only nearby mob when MyHuman is first). Walk the full list.
	auto forEachHuman = [this](auto&& fn)
	{
		for (auto* node = static_cast<TMHuman*>(m_pHumanContainer->m_pDown);
			node != nullptr;
			node = static_cast<TMHuman*>(node->m_pNextLink))
		{
			if (node == m_pMyHuman)
				continue;
			if (fn(node))
	...
	if (g_GameAuto == 1)
	{
		if (m_pAutoTarget && (m_pAutoTarget->m_cDeleted || m_pAutoTarget->m_cDie))
			m_pAutoTarget = nullptr;

		if (!m_pAutoTarget)
		{
			if (forEachHuman([&](TMHuman* pTarget)
				{
			...
			// AutoPostionUse==2 means "don't return to camp", NOT "never chase".
			forEachHuman([&](TMHuman* pNode)
			{
				if (pNode->m_cDeleted || pNode->m_cDie)
					return false;
				int nTX = (int)pNode->m_vecPosition.x;
				int nTY = (int)pNode->m_vecPosition.y;

				if (BASE_GetDistance(nSX, nSY, nTX, nTY) > 12)
					return false;

				const int attack = m_pMyHuman->MAutoAttack(pNode, 1);
				if (attack == 1)
				{
					m_pAutoTarget = pNode;
					return true;
		...
		forEachHuman([&](TMHuman* pNode)
		{
			...
			found = true;
			// Swing first; chase only if out of range (mode 1 returns 0 when already melee).
			int rnt = m_pMyHuman->MAutoAttack(m_pAutoTarget, 0);
			if (rnt != 1)
				rnt = m_pMyHuman->MAutoAttack(m_pAutoTarget, 1);
			if (m_pAutoTarget->m_cShadow == 1 && m_pAutoTarget->m_nClass == 66)
				m_pAutoTarget = nullptr;
			else if (rnt == 0)
			{
				// Fora de alcance / bloqueado: libera para escolher outro no próximo tick.
				const int d = BASE_GetDistance(nSX, nSY,
					(int)m_pAutoTarget->m_vecPosition.x, (int)m_pAutoTarget->m_vecPosition.y);
				if (d > 12)
					m_pAutoTarget = nullptr;
			}
			return true;
	...
	if (g_GameAuto == 2)
	{
		...
		if (!m_pAutoTarget)
		{
			forEachHuman([&](TMHuman* pNode)
			{
				const int nX = (int)pNode->m_vecPosition.x;
				const int nY = (int)pNode->m_vecPosition.y;
				if (AutoSkillUse(nX, nY,
						D3DXVECTOR3(pNode->m_vecPosition.x, pNode->m_fHeight, pNode->m_vecPosition.y),
						0, 0, pNode) == 1)
				{
					m_pAutoTarget = pNode;
					return true;
		...
		forEachHuman([&](TMHuman* pNode)
		{
			...
			found = true;
			const int itx = (int)m_pAutoTarget->m_vecPosition.x;
			const int ity = (int)m_pAutoTarget->m_vecPosition.y;
			if (!AutoSkillUse(itx, ity,
					D3DXVECTOR3(m_pAutoTarget->m_vecPosition.x, m_pAutoTarget->m_fHeight, m_pAutoTarget->m_vecPosition.y),
					0, 0, m_pAutoTarget))
				m_pAutoTarget = nullptr;
			return true;
		});
...
int TMFieldScene::ToggleNativeCCMode(int mode)
{
	...
	char szText[128]{};
	if (g_GameAuto == 0)
		sprintf_s(szText, "C.C desativado");
	else if (g_GameAuto == 1)
		sprintf_s(szText, "C.C fisico ATIVO (tecla A)");
	else
		sprintf_s(szText, "C.C magico ATIVO (tecla D)");
	...
	if (g_GameAuto != 0 && m_pMyHuman && m_pMyHuman->IsInTown() == 1 && m_pChatList)
	{
		auto pTown = new SListBoxItem(
			"C.C so ataca fora da cidade — saia de Armia/zona segura.",
			0xFFFFFF88, 0.0f, 0.0f, 300.0f, 16.0f, 0, 0x77777777u, 1u, 0);
		if (pTown)
			m_pChatList->AddItem(pTown);
	}

	WYD748_DiagnosticsLog("native CC toggle mode=%d g_GameAuto=%d town=%d\r\n",
		mode, g_GameAuto, m_pMyHuman ? m_pMyHuman->IsInTown() : -1);
	return 1;
}

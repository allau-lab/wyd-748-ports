	...
	: TMScene()
{
	...
	g_nBattleMaster = 0;
	LastSendMsgTime = 0;
	m_pSaveAccountCheck = nullptr;
	m_pSaveAccountLabel = nullptr;
	m_pSavedAccountList = nullptr;
	m_pAccountsButton = nullptr;
	m_pAccountsDialog = nullptr;
	m_pAccountsTitle = nullptr;
	m_pAccountsPrev = nullptr;
	m_pAccountsNext = nullptr;
	m_pAccountsSave = nullptr;
	m_pAccountsRemove = nullptr;
	m_pAccountsClose = nullptr;
	m_nSavedAccountCount = 0;
	memset(m_savedAccounts, 0, sizeof(m_savedAccounts));
}
...
int TMSelectServerScene::InitializeScene()
{
	...
	// The legacy logo is part of the server-selection composition and overlaps
	// the 7.48 login controls on SDL3/Wayland. Keep the castle background and
	// render a clean login form; the logo is not part of the login scene.
	for (int i = 0; i < 2; ++i)
	{
		if (m_pLogoPanels[i])
		{
			m_pLogoPanels[i]->SetVisible(0);
			m_pLogoPanels[i]->m_bVisible = 0;
			m_pLogoPanels[i]->m_GCPanel.dwColor = 0;
		}
	}
	...
	// NanumGothic (port) rende as legendas um pouco mais largas que a Gulim
	// original; desloca-as para a esquerda para não encostar nos campos de edição.
	if (m_pLoginIDText)
		m_pLoginIDText->m_nPosX -= 12.0f * RenderDevice::m_fWidthRatio;
	if (m_pLoginPasswordText)
		m_pLoginPasswordText->m_nPosX -= 12.0f * RenderDevice::m_fWidthRatio;
	...
	InitSavedAccountsUI();
	if (m_pSaveAccountLabel)
	{
		m_pSaveAccountLabel->SetVisible(0);
		m_pSaveAccountLabel->m_bVisible = 0;
	}
	LoadSavedAccounts();
	RefreshSavedAccountList();
	if (m_nSavedAccountCount > 0)
		ApplySavedAccount(0);
...
{
	if (idwControlID == kAccountsButtonId)
	{
		if (m_pAccountsDialog)
		{
			const int visible = m_pAccountsDialog->IsVisible() ? 0 : 1;
			m_pAccountsDialog->SetVisible(visible);
			m_pControlContainer->m_pModalControl[0] = visible
				? static_cast<SControl*>(m_pAccountsDialog) : nullptr;
			if (visible)
			{
				m_pControlContainer->SetFocusedControl(nullptr);
				if (m_pSavedAccountList && m_nSavedAccountCount > 0)
					ApplySavedAccount(m_pSavedAccountList->GetSelectedIndex());
			}
		}
		return 1;
	}
	if (idwControlID == kAccountsCloseId)
	{
		if (m_pAccountsDialog)
			m_pAccountsDialog->SetVisible(0);
		m_pControlContainer->m_pModalControl[0] = nullptr;
		return 1;
	}
	if (idwControlID == kAccountsPrevId || idwControlID == kAccountsNextId)
	{
		if (m_pSavedAccountList && m_nSavedAccountCount > 0)
		{
			int index = m_pSavedAccountList->GetSelectedIndex();
			const int delta = idwControlID == kAccountsPrevId ? -1 : 1;
			index = (index + delta + m_nSavedAccountCount) % m_nSavedAccountCount;
			ApplySavedAccount(index);
		}
		return 1;
	}
	if (idwControlID == kAccountsSaveId)
	{
		SaveCurrentAccountIfChecked();
		return 1;
	}
	if (idwControlID == kAccountsRemoveId)
	{
		const int index = m_pSavedAccountList ? m_pSavedAccountList->GetSelectedIndex() : -1;
		if (index >= 0 && index < m_nSavedAccountCount)
		{
			for (int i = index; i + 1 < m_nSavedAccountCount; ++i)
				m_savedAccounts[i] = m_savedAccounts[i + 1];
			--m_nSavedAccountCount;
			FILE* fp = nullptr;
			if (fopen_s(&fp, "SavedAccounts.txt", "wt") == 0 && fp)
			{
				for (int i = 0; i < m_nSavedAccountCount; ++i)
					fprintf(fp, "%s %s\\n", m_savedAccounts[i].name,
						m_savedAccounts[i].password[0] ? m_savedAccounts[i].password : "-");
				fclose(fp);
			}
			RefreshSavedAccountList();
		}
	...
	case B_SERVER_SEL_OK:
	{
		...
		// The login artwork is not part of the login form. Keeping it visible
		// makes the large 7.48 logo cover the account/password controls at 16:10
		// and widescreen modes, so hide it until the user returns to server select.
		for (int i = 0; i < 2; ++i)
		{
			if (m_pLogoPanels[i])
			{
		...
		// The login form is a separate composition. The native logo panels are
		// centered for the server-selection scene and otherwise overlap the fields
		// at wide/asymmetric resolutions (notably 1280x800). Keep the artwork
		// visible, but scale and anchor the two halves as one centered header.
		if (m_pLogoPanels[0] && m_pLogoPanels[1])
		{
			const float logoScale = 0.68f;
			const float leftWidth = m_pLogoPanels[0]->m_nWidth * logoScale;
			const float rightWidth = m_pLogoPanels[1]->m_nWidth * logoScale;
			const float headerWidth = leftWidth + rightWidth;
			const float headerX = (static_cast<float>(g_pDevice->m_dwScreenWidth) - headerWidth) * 0.5f;
			const float headerY = 12.0f * RenderDevice::m_fHeightRatio;
			m_pLogoPanels[0]->SetRealSize(leftWidth, m_pLogoPanels[0]->m_nHeight * logoScale);
			m_pLogoPanels[1]->SetRealSize(rightWidth, m_pLogoPanels[1]->m_nHeight * logoScale);
			m_pLogoPanels[0]->SetRealPos(headerX, headerY);
			m_pLogoPanels[1]->SetRealPos(headerX + leftWidth, headerY);
		}
	...
	if (idwControlID == kSavedAccountListId)
	{
		if (m_pSavedAccountList)
			ApplySavedAccount(m_pSavedAccountList->GetSelectedIndex());
		return 1;
	}
...
int TMSelectServerScene::OnPacketEvent(unsigned int dwCode, char* buf)
{
	...
	// Login confirmation is a scene transition, not a generic scene message.
	// Handle it before TMScene: the generic control tree may consume/short-circuit
	// an otherwise valid 0x10A frame, leaving the UI stuck on "Now connecting".
	if (packet->Type == MSG_CNFAccountLogin_Opcode)
	{
		if (packet->Size < sizeof(MSG_CNFAccountLogin))
			return 1;
...
int TMSelectServerScene::FrameMove(unsigned int dwServerTime)
{
	...
	else if (m_cLogin == 1)
	{
		...
		// Enforce the login composition every frame. Some RCBIN controls
		// rebuild their geometry during FrameMove and can otherwise re-enable the
		// selection logo after the scene transition.
		if (m_pNServerSelect)
		{
			m_pNServerSelect->SetVisible(0);
			m_pNServerSelect->m_GCPanel.dwColor = 0;
		}
		for (int i = 0; i < 2; ++i)
		{
			if (m_pLogoPanels[i])
	...
	if (m_pNServerGroupList)
	{
		...
		for (int i = m_nMaxGroup; i >= 0; --i)
		{
			...
			if (count > -1 && g_pServerList[count][0][0])
			{
				...
				pGroupItem[i] = new SListBoxItem(szStr, 0xFFFFFFFF, 0.0f, 0.0f, 63.0f, 16.0f, 0, 0x77777777u, 1u, 0);
				...
				m_pGroupPanel[i] = new SPanel(-395, 17.0f,
					...
					0x01010101, RENDERCTRLTYPE::RENDER_IMAGE_STRETCH);
				m_pNServerSelect->AddChild(static_cast<TreeNode*>(m_pGroupPanel[i]));
				// -395 is a legacy selection overlay whose atlas entry is an opaque
				// orange strip on SDL3. The list item already owns its native selected
				// state; keep the compatibility panel only for hit/layout ownership.
				m_pGroupPanel[i]->SetVisible(0);
			}
		}
	}
...
void TMSelectServerScene::InitSavedAccountsUI()
{
	if (!m_pLoginPanel || !m_pControlContainer)
		return;

	// trade_check (set 33): índice 0 = off, 1 = on — mesmo atlas do cliente clássico.
	m_pSaveAccountCheck = new SCheckBox(33, 12.0f, 118.0f, 16.0f, 16.0f, 0xFFFFFFFF);
	m_pSaveAccountCheck->SetControlID(kSaveAccountCheckId);
	m_pSaveAccountCheck->SetEventListener(static_cast<IEventListener*>(m_pControlContainer));
	m_pSaveAccountCheck->SetValue(1);
	m_pLoginPanel->AddChild(static_cast<TreeNode*>(m_pSaveAccountCheck));

	// Child controls use coordinates relative to m_pLoginPanel. The previous
	// code reused the button's already-materialized absolute Y coordinate,
	// adding the panel origin a second time and placing this label below the
	// form at 1280x800. Keep it on the checkbox row in panel-local coordinates.
	float fLabelX = 34.0f;
	float fLabelY = 118.0f;
	m_pSaveAccountLabel = new SText(-2, "Salvar conta", 0xFFFFFFFF,
		fLabelX, fLabelY, 140.0f, 16.0f, 0, 0, 0, 0);
	m_pSaveAccountLabel->SetControlID(900103u);
	// The checkbox is self-explanatory in the native 7.48 panel. The old
	// standalone caption was rendered in absolute coordinates and floated below
	// the form on Linux, so do not draw a second, detached label.
	m_pLoginPanel->AddChild(static_cast<TreeNode*>(m_pSaveAccountLabel));
	m_pSaveAccountLabel->SetVisible(0);
	m_pSaveAccountLabel->m_bVisible = 0;

	// Account manager: use a dedicated modal anchored to the viewport instead
	// of placing the list inside the login form. The old child-relative layout
	// drifted behind the server panel at widescreen resolutions.
	m_pAccountsButton = new SButton(14, 16.0f, 145.0f, 96.0f, 24.0f,
		0, 1, const_cast<char*>("CONTAS"));
	m_pAccountsButton->SetControlID(kAccountsButtonId);
	m_pAccountsButton->SetEventListener(static_cast<IEventListener*>(m_pControlContainer));
	m_pLoginPanel->AddChild(static_cast<TreeNode*>(m_pAccountsButton));

	// Use the color-only panel path. The UI atlas entry used by this custom
	// modal is white on Linux, while the original account picker is dark glass.
	m_pAccountsDialog = new SPanel(-2, 0.0f, 0.0f, 340.0f, 380.0f,
		0xE61C1715u, RENDERCTRLTYPE::RENDER_IMAGE_STRETCH);
	m_pAccountsDialog->SetControlID(900110u);
	m_pAccountsDialog->m_bModal = 1;
	m_pAccountsDialog->SetPos(
		(static_cast<float>(g_pDevice->m_dwScreenWidth) - m_pAccountsDialog->m_nWidth) * 0.5f,
		(static_cast<float>(g_pDevice->m_dwScreenHeight) - m_pAccountsDialog->m_nHeight) * 0.5f);
	m_pControlContainer->AddItem(static_cast<SControl*>(m_pAccountsDialog));

	m_pAccountsTitle = new SText(-1, const_cast<char*>("CONTAS"), 0xFFFFE8C2u, 0.0f, 14.0f,
		340.0f, 22.0f, -1, 1, 0, 1);
	m_pAccountsTitle->m_dwAlignType = 1;
	m_pAccountsDialog->AddChild(static_cast<TreeNode*>(m_pAccountsTitle));

	m_pSavedAccountList = new SListBox(-2, kMaxSavedAccounts, 5,
		14.0f, 52.0f, 312.0f, 210.0f, 0xCC302820u,
		RENDERCTRLTYPE::RENDER_IMAGE_STRETCH, 1, 0, 0);
	m_pSavedAccountList->SetControlID(kSavedAccountListId);
	m_pSavedAccountList->SetEventListener(static_cast<IEventListener*>(m_pControlContainer));
	m_pAccountsDialog->AddChild(static_cast<TreeNode*>(m_pSavedAccountList));

	m_pAccountsPrev = new SButton(14, 14.0f, 274.0f, 42.0f, 24.0f, 0, 1, const_cast<char*>("<"));
	m_pAccountsNext = new SButton(14, 284.0f, 274.0f, 42.0f, 24.0f, 0, 1, const_cast<char*>(">"));
	m_pAccountsSave = new SButton(14, 14.0f, 326.0f, 98.0f, 24.0f, 0, 1, const_cast<char*>("SALVAR"));
	m_pAccountsRemove = new SButton(14, 121.0f, 326.0f, 98.0f, 24.0f, 0, 1, const_cast<char*>("REMOVER"));
	m_pAccountsClose = new SButton(15, 232.0f, 326.0f, 94.0f, 24.0f, 0, 1, const_cast<char*>("FECHAR"));
	SButton* accountButtons[] = { m_pAccountsPrev, m_pAccountsNext, m_pAccountsSave,
		m_pAccountsRemove, m_pAccountsClose };
	const unsigned int accountIds[] = { kAccountsPrevId, kAccountsNextId,
		kAccountsSaveId, kAccountsRemoveId, kAccountsCloseId };
	for (size_t i = 0; i < sizeof(accountButtons) / sizeof(accountButtons[0]); ++i)
	{
		accountButtons[i]->SetControlID(accountIds[i]);
		accountButtons[i]->SetEventListener(static_cast<IEventListener*>(m_pControlContainer));
		accountButtons[i]->m_GCPanel.dwColor = 0x01010101u;
		m_pAccountsDialog->AddChild(static_cast<TreeNode*>(accountButtons[i]));
	}
	m_pAccountsDialog->SetVisible(0);
}

void TMSelectServerScene::LoadSavedAccounts()
{
	m_nSavedAccountCount = 0;
	memset(m_savedAccounts, 0, sizeof(m_savedAccounts));

	FILE* fp = nullptr;
	if (fopen_s(&fp, "SavedAccounts.txt", "rt") != 0 || !fp)
		return;

	char line[128]{};
	while (fgets(line, sizeof(line), fp) && m_nSavedAccountCount < kMaxSavedAccounts)
	{
		char name[32]{};
		char pass[32]{};
		if (sscanf(line, "%31s %31s", name, pass) < 1 || !name[0])
			continue;

		strncpy(m_savedAccounts[m_nSavedAccountCount].name, name,
			sizeof(m_savedAccounts[m_nSavedAccountCount].name) - 1);
		if (pass[0])
		{
			strncpy(m_savedAccounts[m_nSavedAccountCount].password, pass,
				sizeof(m_savedAccounts[m_nSavedAccountCount].password) - 1);
		}
		++m_nSavedAccountCount;
	}
	fclose(fp);
...
void TMSelectServerScene::SaveCurrentAccountIfChecked()
{
	if (!m_pSaveAccountCheck || m_pSaveAccountCheck->GetValue() != 1)
		return;
	if (!m_pEditID || !m_pEditPW)
		return;

	const char* name = m_pEditID->GetText();
	const char* pass = m_pEditPW->GetText();
	if (!name || !name[0] || strlen(name) < 4)
		return;

	int found = -1;
	for (int i = 0; i < m_nSavedAccountCount; ++i)
	{
		if (!_stricmp(m_savedAccounts[i].name, name))
		{
			found = i;
			break;
		}
	...
	if (pass)
		strncpy(entry.password, pass, sizeof(entry.password) - 1);

	if (found >= 0)
	{
		m_savedAccounts[found] = entry;
	}
	else if (m_nSavedAccountCount < kMaxSavedAccounts)
	{
		m_savedAccounts[m_nSavedAccountCount++] = entry;
	}
	else
	{
		// Lista cheia: substitui a mais antiga e coloca a nova no topo.
		for (int i = 0; i < kMaxSavedAccounts - 1; ++i)
			m_savedAccounts[i] = m_savedAccounts[i + 1];
		m_savedAccounts[kMaxSavedAccounts - 1] = entry;
	}

	// Conta usada agora sobe para o topo da lista.
	if (found > 0)
	{
		const SavedAccount top = m_savedAccounts[found];
		for (int i = found; i > 0; --i)
			m_savedAccounts[i] = m_savedAccounts[i - 1];
		m_savedAccounts[0] = top;
	}
	else if (found < 0 && m_nSavedAccountCount > 1)
	{
		const SavedAccount top = m_savedAccounts[m_nSavedAccountCount - 1];
		for (int i = m_nSavedAccountCount - 1; i > 0; --i)
			m_savedAccounts[i] = m_savedAccounts[i - 1];
		m_savedAccounts[0] = top;
	}
...
void TMSelectServerScene::ApplySavedAccount(int index)
{
	if (index < 0 || index >= m_nSavedAccountCount)
		return;
	if (!m_pEditID || !m_pEditPW)
	...
	m_pEditID->SetText(m_savedAccounts[index].name);
	if (m_savedAccounts[index].password[0] && strcmp(m_savedAccounts[index].password, "-") != 0)
		m_pEditPW->SetText(m_savedAccounts[index].password);
	else
		m_pEditPW->SetText(const_cast<char*>(""));

	if (m_pSavedAccountList)
		m_pSavedAccountList->SetSelectedIndex(index);
}

void TMSelectServerScene::RefreshSavedAccountList()
{
	if (!m_pSavedAccountList)
		return;

	m_pSavedAccountList->Empty();
	for (int i = 0; i < m_nSavedAccountCount; ++i)
	{
		auto* item = new SListBoxItem(m_savedAccounts[i].name, 0xFFFFFFFF,
			0.0f, 0.0f, 120.0f, 16.0f, 0, 0x77777777u, 1u, 0);
		m_pSavedAccountList->AddItem(item);
	}		m_pSavedAccountList->SetVisible(m_nSavedAccountCount > 0 ? 1 : 0);
	if (m_nSavedAccountCount > 0)
		m_pSavedAccountList->SetSelectedIndex(0);
	if (m_pAccountsTitle)
	{
		char title[64]{};
		snprintf(title, sizeof(title), "Contas %d/%d", m_nSavedAccountCount > 0 ? 1 : 0,
			m_nSavedAccountCount);
		m_pAccountsTitle->SetText(title, 0);
	}
}

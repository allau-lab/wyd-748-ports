...
int TMSelectCharScene::InitializeScene()
{
	...
	g_pDevice->m_dwClearColor = 0x555544;
	g_pDevice->m_bFog = 0;
	g_pDevice->m_colorLight.r = 1.0f;
	g_pDevice->m_colorLight.g = 1.0f;
	g_pDevice->m_colorLight.b = 1.0f;
	g_pDevice->m_colorLight.a = 1.0f;
	// Back light mais forte: personagem/selchar no DXVK depende bastante do
	// emissive derivado de m_colorBackLight.
	g_pDevice->m_colorBackLight.r = 0.85f;
	g_pDevice->m_colorBackLight.g = 0.85f;
	g_pDevice->m_colorBackLight.b = 0.85f;
	g_pDevice->m_colorBackLight.a = 1.0f;
	if (g_pDevice->m_pd3dDevice)
		g_pDevice->m_pd3dDevice->SetRenderState(D3DRS_AMBIENT, 0xFFCCCCCC);
	...
	if (m_pObjectContainerList[0]->Load(szDataPath))
	{
...
#ifdef __linux__
		// Estado 3 (noite) + LightVal ~0.26 deixa o personagem preto no DXVK
		// (membros somem no fundo). Dia (0) mantém a cena legível.
		...
		for (int i = 0; i < 10; ++i)
		{
			...
			if (m_pBtnNumDlg[i])
			{
				// ResourceControl IDs can be remapped by the SDL3 loader. Restore
				// the semantic ID explicitly so the Linux skin policy is deterministic.
				m_pBtnNumDlg[i]->SetControlID(static_cast<unsigned int>(i + 66437));
				// The SDL3/DXVK path has no valid texture for the later-client
				// numeric button atlas. Its white fallback was covering the native
				// dark PIN2 skin. Keep the button hitbox/text, but let the original
				// panel texture show through.
				m_pBtnNumDlg[i]->m_GCPanel.dwColor = 0x01010101;
			}
		}
		...
		// The three action buttons use the same missing later-atlas entry.
		// Preserve their native dark skin and white labels instead of painting
		// opaque white rectangles over the dialog.
		for (unsigned int id = 66433u; id <= 66435u; ++id)
		{
			if (auto* button = dynamic_cast<SButton*>(m_pControlContainer->FindControl(id)))
			{
				button->SetControlID(id);
				button->m_GCPanel.dwColor = 0x01010101;
			}
		}
		...
		// RC coordinates are already converted to the active SDL3 viewport by
		// SControl. Do not apply a second pixel offset here; it separates the
		// title from the character panel on widescreen displays.
		...
		if (m_pSelbackpanel)
		{
			// Keep the native bottom-left anchor. The previous extra 30px shift
			// was applied in addition to the later button layout and made the
			// create/return controls drift or clip at 16:9 and 16:10 ratios.
...
int TMSelectCharScene::OnControlEvent(unsigned int idwControlID, unsigned int idwEvent)
{
	...
	if (idwControlID == 66433)
	{
		...
		if (!g_AccountLock)
		{
			...
			strncpy(msLock.ItemPassWord, keypass, sizeof(msLock.ItemPassWord) - 1); msLock.ItemPassWord[sizeof(msLock.ItemPassWord) - 1] = 0; /* was strlen */
	...
	if (idwControlID == 66435)
	{
		...
		else if (g_AccountLock == 4)
		{
			...
			strncpy(keypasschage, keypass, sizeof(keypasschage) - 1); keypasschage[sizeof(keypasschage) - 1] = 0;
...
int TMSelectCharScene::OnMouseEvent(unsigned int dwFlags, unsigned int wParam, int nX, int nY)
{
	...
	if (m_bSelect == 1)
	{
		...
		sprintf(szValue, WYD_FMT_I64, g_pObjectManager->m_stSelCharData.Exp[i]);
		...
		sprintf_s(szValue, WYD_FMT_I64, levelTable[nextLevelIndex]);
...
void TMSelectCharScene::VisibleSelectCreate(int bSelect)
{
	...
	if (bSelect != 1)
	{
		...
		if (pCreWin)
		{
			// Keep the creation form in the right-hand safe area. Use its actual
			// scaled dimensions rather than the old fixed 514px origin.
			const float rightAreaCenter = static_cast<float>(g_pDevice->m_dwScreenWidth) * 0.72f;
			pCreWin->SetPos(rightAreaCenter - pCreWin->m_nWidth * 0.5f,
				static_cast<float>(g_pDevice->m_dwScreenHeight) * 0.5f - pCreWin->m_nHeight * 0.5f);
		}
	...
	else
	{
		...
		// These are root-level controls in SelCharScene2, not children of the
		// decorative anchor. Position all three from one viewport anchor so the
		// labels and hitboxes cannot split when the SDL3 window is non-4:3.
		const float left = 30.0f * RenderDevice::m_fWidthRatio;
		const float bottom = 24.0f * RenderDevice::m_fHeightRatio;
		const float gap = 6.0f * RenderDevice::m_fHeightRatio;
		...
		// In SelCharScene2.bin the captioned buttons (4613 Create, 4615 Delete,
		// 5673 Esc) are children at (0,0) of the panels positioned above, so they
		// follow them. 4616 (Cancel) and 5653 (Location) belong to panel 1282.
...
void TMSelectCharScene::AddvirtualKeyNum(int num)
{
	...
	if (paslen < 6)
	{
		...
		strncpy(chdata, Passdata, sizeof(chdata) - 1); chdata[sizeof(chdata) - 1] = 0;

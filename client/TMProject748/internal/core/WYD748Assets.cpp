...
bool WYD748_InitializeAssetRoot()
{
    ...
    // A real client root carries ItemList.bin at the top level; probing it
    // avoids chdir'ing into a build/ directory that has no game data.
    auto looksLikeGameRoot = [](const char* dir) {
        if (!dir || !dir[0])
            return false;
        const std::string probe = std::string(dir) + "/ItemList.bin";
        FILE* f = nullptr;
        if (fopen_s(&f, probe.c_str(), "rb") != 0 || !f)
            return false;
        std::fclose(f);
        return true;
    };
    ...
    const DWORD exeLen = GetModuleFileNameA(nullptr, exeDir, MAX_PATH);
    char* separator = exeLen > 0 ? strrchr(exeDir, '/') : nullptr;
    if (separator == nullptr)
        separator = exeLen > 0 ? strrchr(exeDir, '\\') : nullptr;
    if (separator != nullptr)
        *separator = '\0';
    else
        exeDir[0] = '\0';

    // 1) Diretório do executável com dados (binário colado no client, Docker).
    if (exeDir[0] != '\0' && looksLikeGameRoot(exeDir))
        return SetCurrentDirectoryA(exeDir) != FALSE;

    // 2) Diretório de onde o usuário lançou (build host: client748).
    char cwd[MAX_PATH]{};
    if (GetCurrentDirectoryA(MAX_PATH, cwd) != 0 && cwd[0] != '\0' && looksLikeGameRoot(cwd))
        return TRUE; // já estamos na raiz correta

    // 3) Fallback histórico: diretório do executável.
    if (exeDir[0] != '\0')
        return SetCurrentDirectoryA(exeDir) != FALSE;
    return false;
}
    ...
    const std::size_t valueCount)
{
    ...
    while (fgets(line, static_cast<int>(sizeof(line)), file) != nullptr)
    {
        ...
        if (keyIndex == _countof(keys))
        {
            // Optional flags (LoL controls, etc.) may appear without invalidating
            // the required 7.48 key set — they are read via LoadOptionalClientInt.
            if (strcmp(key, "LOL_CONTROLS") == 0)
                continue;
            if (strcmp(key, "TOUCH_UI") == 0)
                continue;
            valid = false;
            continue;
        }

        if (seen[keyIndex] || value < SHRT_MIN || value > SHRT_MAX)
        {
            valid = false;
            continue;
        }
...
bool WYD748_LoadOptionalClientInt(
    const char* path,
    const char* key,
    int* outValue)
{
    if (path == nullptr || key == nullptr || outValue == nullptr)
        return false;
    ...
    while (fgets(line, static_cast<int>(sizeof(line)), file) != nullptr)
    {
        ...
        int value = 0;
#ifdef WYD_LINUX
        if (sscanf(line, " [%31[^]]] %d", parsedKey, &value) != 2)
#else
        if (sscanf_s(line, " [%31[^]]] %d", parsedKey, static_cast<unsigned int>(_countof(parsedKey)), &value) != 2)
#endif
            continue;

        if (strcmp(parsedKey, key) != 0)
            continue;

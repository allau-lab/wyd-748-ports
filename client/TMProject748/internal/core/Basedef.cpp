...
#ifdef WYD_LINUX
			int ret = sscanf(str, "%d %63s", &Index, part);
#else
...
int BASE_InitializeServerList()
{
	...
	if (fpBin)
	{
		// Chave do binário Win32 (CP1252/latin-1). Em source UTF-8 o literal
		// "¤¡¤¤..." vira 128 bytes e truncar a 64 quebra a descriptografia.
		static const unsigned char kServerListKey[64] = {
		...
		char szList[65];
		memcpy(szList, kServerListKey, 64);
		szList[64] = 0;
...
#ifdef WYD_LINUX
		std::fprintf(stderr, "[WYDLINUX] serverlist [0][0]=\"%s\" [0][1]=\"%s\" [0][2]=\"%s\"\n",
			g_pServerList[0][0], g_pServerList[0][1], g_pServerList[0][2]);
#endif
		return 1;
	}

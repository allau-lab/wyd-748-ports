...
public:
    ...
    // LoL-style swing: movement is blocked only during windup; recovery is cancelable.
    bool IsAttackSwingMotion() const;
    bool IsInCombatWindup(unsigned int dwServerTime) const;
    bool CanCancelCombatWithMove(unsigned int dwServerTime) const;
    bool BlocksMoveDuringAttackSwing(unsigned int dwServerTime, bool holdingPrimaryAttack) const;
...
public:
    ...
    TMEffectBillBoard* m_pEyeFire[11];
    TMEffectBillBoard* m_pEyeFire2[11];
    ...
    // Attack swing windows (ms server time). Zero when not in an attack motion.
    unsigned int m_dwCombatWindupEnd;
    unsigned int m_dwCombatRecoveryEnd;

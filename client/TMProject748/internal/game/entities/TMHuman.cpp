...
TMHuman::TMHuman(TMScene* pParentScene)
{
    ...
    if (g_pCurrentScene != nullptr)
    {
        ...
        m_pProgressBar = new SProgressBar(-2, 30, 30, 0.0f, 0.0f, 72.0f, 7.0f,
#if defined(__SWITCH__)
            0xFF55FF55u, 0xFF222222u, 1u);
#else
            0xFF9EF048, 0xFF333333, 1u);
#endif
        m_pProgressBar1 = new SProgressBar(-2, 30, 30, 0.0f, 0.0f, 72.0f, 7.0f,
#if defined(__SWITCH__)
            0xFF55AAFFu, 0xFF222222u, 1u);
#else
            0xFF9EF048, 0xFF333333, 1u);
#endif
...
int TMHuman::FrameMove(unsigned int dwServerTime)
{
    ...
    // Punch/flinch no mesmo instante do pacote de dano (sem +200 ms extras).
    if (m_stPunchEvent.dwTime && dwServerTime >= m_stPunchEvent.dwTime)
    {
...
int TMHuman::OnPacketUpdateScore(MSG_STANDARD* pStd)
{
    ...
    if (g_pCurrentScene->m_eSceneType == ESCENE_TYPE::ESCENE_FIELD)
    {
        ...
        // Compat 7.48: Japanese bag-page UI is absent; Bag_View is a no-op there.
        if (!pField->m_bCompatFieldScene)
            pField->Bag_View();
    }
...
void TMHuman::Init()
{
    ...
    m_dwCombatWindupEnd = 0;
    m_dwCombatRecoveryEnd = 0;
    m_dwLastDummyTime = 0;
    m_dwWaterTime = 0;
    m_dwLastHaste = 0;
    m_dwElimental = 0;
...
void TMHuman::UpdateScore(int nGuildLevel)
{
    if (!m_dwDelayDel)
    {
        ...
        if (pScene && pScene->m_pControlContainer && pScene->m_pMyHuman)
        ...
        if (pScene && g_pCurrentScene->GetSceneType() == ESCENE_TYPE::ESCENE_FIELD && pScene->m_pMyHuman == this)
        {
...
void TMHuman::SetAnimation(ECHAR_MOTION eMotion, int nLoop)
{
    if (!m_dwDelayDel && eMotion != ECHAR_MOTION::ECMOTION_NONE && m_pSkinMesh)
    {
        ...
        // Attack motions 4..9: split swing into windup (uncancelable) and recovery.
        if ((int)eMotion >= 4 && (int)eMotion <= 9 && m_pSkinMesh)
        {
            unsigned int dwMod = MeshManager::m_BoneAnimationList[m_nSkinMeshType].numAniCut[m_pSkinMesh->m_nAniIndex];
            if (dwMod > 2)
                dwMod -= 2;
            const unsigned int totalMs = 4u * dwMod * m_pSkinMesh->m_dwFPS;
            // ~40% commitment window; remainder is cancelable for orbwalk.
            m_dwCombatWindupEnd = m_dwStartAnimationTime + (totalMs * 2u) / 5u;
            m_dwCombatRecoveryEnd = m_dwStartAnimationTime + totalMs;
        }
        else
        {
            m_dwCombatWindupEnd = 0;
...
bool TMHuman::IsAttackSwingMotion() const
{
    return (int)m_eMotion >= 4 && (int)m_eMotion <= 9;
}

bool TMHuman::IsInCombatWindup(unsigned int dwServerTime) const
{
    return IsAttackSwingMotion() && m_dwCombatWindupEnd != 0 && dwServerTime < m_dwCombatWindupEnd;
}

bool TMHuman::CanCancelCombatWithMove(unsigned int dwServerTime) const
{
    if (!IsAttackSwingMotion())
        return true;
    if (!g_bLolControls)
        return true;
    return m_dwCombatWindupEnd == 0 || dwServerTime >= m_dwCombatWindupEnd;
}

bool TMHuman::BlocksMoveDuringAttackSwing(unsigned int dwServerTime, bool holdingPrimaryAttack) const
{
    if (!IsAttackSwingMotion() || !m_pSkinMesh)
        return false;

    if (g_bLolControls)
        return IsInCombatWindup(dwServerTime);

    if (!holdingPrimaryAttack)
        return false;

    unsigned int dwMod = MeshManager::m_BoneAnimationList[m_nSkinMeshType].numAniCut[m_pSkinMesh->m_nAniIndex];
    if (dwMod > 2)
        dwMod -= 2;
    return dwServerTime < m_dwStartAnimationTime + 4u * dwMod * m_pSkinMesh->m_dwFPS;
}
...
void TMHuman::SetColorMaterial()
{
    ...
    else
    {
        if (m_bSelected)
        {
            ...
            m_pSkinMesh->m_materials.Diffuse.a = 1.0f;
            m_pSkinMesh->m_materials.Specular.r = 1.0f;
            m_pSkinMesh->m_materials.Specular.g = 1.0f;
            m_pSkinMesh->m_materials.Specular.b = 1.0f;
            m_pSkinMesh->m_materials.Specular.a = 1.0f;
            m_pSkinMesh->m_materials.Emissive.r = 1.0f;
            m_pSkinMesh->m_materials.Emissive.g = 1.0f;
            m_pSkinMesh->m_materials.Emissive.b = 1.0f;
            m_pSkinMesh->m_materials.Emissive.a = 1.0f;
        }
        ...
        else
        {
            ...
            // Zero-init left a=0 → DXVK SRCALPHA made selchar limbs invisible.
            color.a = 1.0f;
                ...
                else
                {
                    ...
                    if (pFocused)
                    {
...
#if defined(__SWITCH__)
                        // Cores nativas (1E821E / SlateBlue) ficam quase pretas no OLED
                        // do Switch; usar saturadas legíveis sobre o personagem.
                        int blueblue = 0xFF55AAFFu;
                        const unsigned int allyHp = 0xFF44EE44u;
                        const unsigned int selfHp = 0xFF55FF55u;
                        const unsigned int enemyHp = 0xFFFF3333u;
#else
                        int blueblue = SlateBlue;
                        const unsigned int allyHp = 0xFF1E821Eu;
                        const unsigned int selfHp = 0xFF1E821Eu;
                        const unsigned int enemyHp = 0xFFFF0000u;
#endif
                        m_pProgressBar->m_GCProgress.dwColor = enemyHp;
                        m_pProgressBar1->m_GCProgress.dwColor = blueblue;

                        if (g_bCastleWar)
                        {
                            if (pFocused->m_cMantua > 0 && m_cMantua > 0 && pFocused->m_cMantua == m_cMantua)
                                m_pProgressBar->m_GCProgress.dwColor = allyHp;
                            m_pProgressBar1->m_GCProgress.dwColor = blueblue;
                        }
                        ...
                        if (IsInCastleZone2() && !g_bCastleWar)
                        {
                            m_pProgressBar->m_GCProgress.dwColor = enemyHp;
                            m_pProgressBar1->m_GCProgress.dwColor = blueblue;
                            m_pTitleProgressBar->m_GCProgress.dwColor = enemyHp;
                        }
                        if (pFocused == this || m_bParty == 1 || m_usGuild && 
                            (m_usGuild == pFocused->m_usGuild || g_pObjectManager->m_usAllyGuild == m_usGuild))
                        {
                            m_pProgressBar->m_GCProgress.dwColor = selfHp;
                            m_pProgressBar1->m_GCProgress.dwColor = blueblue;
                            m_pTitleProgressBar->m_GCProgress.dwColor = selfHp;
                        }
                    }
                }
            }
...
#if defined(__SWITCH__)
                        const unsigned int mpCol = 0xFF55AAFFu;
                        const unsigned int allyHp = 0xFF44EE44u;
                        const unsigned int selfHp = 0xFF55FF55u;
                        const unsigned int enemyHp = 0xFFFF3333u;
#else
                        const unsigned int mpCol = Blue;
                        const unsigned int allyHp = 0xFF1E821Eu;
                        const unsigned int selfHp = 0xFF1E821Eu;
                        const unsigned int enemyHp = 0xFFFF0000u;
#endif
                        m_pProgressBar->m_GCProgress.dwColor = enemyHp;
                        m_pProgressBar1->m_GCProgress.dwColor = mpCol;
...
void TMHuman::GetRoute(IVector2 vecTarget, int nCount, int bStop)
{
    ...
    // LoL: ignore repath during attack windup (recovery may cancel the swing).
    if (g_bLolControls
        && g_pCurrentScene
        && this == g_pCurrentScene->m_pMyHuman
        && IsAttackSwingMotion())
    {
        const unsigned int now = g_pTimerManager->GetServerTime();
        if (!CanCancelCombatWithMove(now))
            return;
    }

    if (vecTarget.x != m_LastSendTargetPos.x || vecTarget.y != m_LastSendTargetPos.y)
    {
        int nStartRouteIndex = m_nLastRouteIndex;
        if (m_fProgressRate > 0.5f)
            nStartRouteIndex = m_nLastRouteIndex + 1;
        ...
        // Buffer de rota stale/zero (comum após teleporte) quebrava C.C chase.
        if (abs(nSX - (int)m_vecPosition.x) > 3 || abs(nSY - (int)m_vecPosition.y) > 3)
        {
            nSX = (int)m_vecPosition.x;
            nSY = (int)m_vecPosition.y;
        }
                ...
                if (nHeight2 <= 30 || !m_LastSendTargetPos.x || !m_LastSendTargetPos.y)
                {
                    ...
                    else if (cRouteBuffer[0] != 0)
                    {
                        ...
                        if (g_pCurrentScene->m_eSceneType != ESCENE_TYPE::ESCENE_DEMO)
                        {
                            ...
                            {
                                bFind = false;
                                for (TMHuman* pNode = (TMHuman*)pScene->m_pHumanContainer->m_pDown; pNode; pNode = (TMHuman*)pNode->m_pNextLink)
                                {
...
void TMHuman::SetCharHeight(float fCon)
{
    ...
    else
    {
        ...
        // Field mobs with mis-aligned Score.Con (or trade-sized Con) explode
        // visually; clamp before the (Con/2000+1)*0.9 formula.
        if (fCon > 4000.0f)
            fCon = 4000.0f;
#endif
    }
...
int TMHuman::MAutoAttack(TMHuman* pTarget, int mode)
{
    ...
    {
        auto pFocused = g_pCurrentScene->m_pMyHuman;
        if (pFocused && m_cMantua > 0 && pFocused->m_cMantua > 0 && m_cMantua == pFocused->m_cMantua)
            return 0;
    }
    ...
    int nSX = (int)m_vecPosition.x;
    int nSY = (int)m_vecPosition.y;
    const int targetCellX = (int)pTarget->m_vecPosition.x;
    const int targetCellY = (int)pTarget->m_vecPosition.y;
    int nTX = targetCellX;
    int nTY = targetCellY;
    int nDistance = BASE_GetDistance(nSX, nSY, nTX, nTY);
    int nMobAttackRange = nSpecForce + BASE_GetMobAbility(&g_pObjectManager->m_stMobData, 27);
    ...
    // Chunk-edge / corrupt height samples often zero the LOS result and used to
    // permanently silence C.C. Prefer clear LOS when available; still swing when
    // adjacent or within melee.
    bool losClear = true;
    if (nDistance > 2)
    {
        int hitX = targetCellX;
        int hitY = targetCellY;
        BASE_GetHitPosition(nSX, nSY, &hitX, &hitY, (char*)pScene->m_HeightMapData, 8);
        if (!hitX || !hitY || hitX != targetCellX || hitY != targetCellY)
            losClear = false;
        nTX = hitX;
        nTY = hitY;
    }
    ...
    // Idle: force LastSend to the cell we occupy so mode-0 / C.C can swing.
    // Nunca sobrescrever LastSend no meio do passo — desincroniza o servidor.
    if (m_fProgressRate <= 0.05f)
    {
        m_LastSendTargetPos.x = nSX;
        m_LastSendTargetPos.y = nSY;
    }

    const bool standing =
        m_LastSendTargetPos.x == nSX && m_LastSendTargetPos.y == nSY;
    const bool inMelee = nDistance <= nMobAttackRange && (losClear || nDistance <= 2);
    const bool canSwing = inMelee && standing;
    ...
    if (canSwing)
    {
        ...
        stAttack.PosX = nSX;
        stAttack.PosY = nSY;
        // Só reutiliza MoveStop se ainda estiver perto da posição real (evita Pos inválido).
        if (pScene->m_stMoveStop.NextX &&
            abs(pScene->m_stMoveStop.NextX - nSX) <= 2 &&
            abs(pScene->m_stMoveStop.NextY - nSY) <= 2)
        {
            stAttack.PosX = pScene->m_stMoveStop.NextX;
            stAttack.PosY = pScene->m_stMoveStop.NextY;
        }
        ...
        if (!pTargetHuman)
            pTargetHuman = pTarget;
        if (!pTargetHuman || pTargetHuman->m_cDeleted)
            return 0;
                ...
                if (pNode->m_dwID > 0 && pNode->m_dwID < 1000)
                {
                    ...
                    if (!TMFieldScene::m_bPK)
                    {
                        ...
                        else
                        {
                            ...
                            if ((m_cMantua == 3 && (pNode->m_dwID > 0 && pNode->m_dwID < 1000)) ||
    ...
    if (!mode)
        return 0;
    // Manual shift-walk cancel must not block C.C chase.
    if (!g_GameAuto && (GetKeyState(VK_SHIFT) >> 8) > 0)
        return 0;

    // Já em alcance: não dispara GetRoute inútil (travava LastSend).
    if (inMelee)
        return 0;
    ...
    while (nMoveDistance2 > nMobAttackRange)
    {
        ...
        if (nMoveSY != nTY)
            nMoveSY += PlusY;
        nMoveDistance2 = BASE_GetDistance(nMoveSX, nMoveSY, nTX, nTY);
        // Guarda contra loop infinito se PlusX/Y zerarem sem aproximar.
        if (nMoveSX == nTX && nMoveSY == nTY)
            break;
    }

...
int TMObject::Render()
{
	...
	if (!RenderDevice::m_bDungeon || RenderDevice::m_bDungeon == 3 || RenderDevice::m_bDungeon == 4)
	{
		...
		color.b = 0.69999999f;
		color.a = 1.0f;
		// Selchar: piso usa luz direcional + fog; no DXVK o difuso some e o
		// chão fica preto. Emissive alto compensa ambient/luz fracos no Native.
		const bool selchar = pScene != nullptr
			&& pScene->m_eSceneType == ESCENE_TYPE::ESCENE_SELCHAR;
		const float em = selchar ? 0.85f : 0.30000001f;
		materials.Emissive.r = em;
		materials.Emissive.g = em;
		materials.Emissive.b = em;
...
int TMObject::IsInTown()
{
	...
	TMObject* pFocused = g_pObjectManager && g_pObjectManager->m_pCamera
		? g_pObjectManager->m_pCamera->GetFocusedObject()
		: nullptr;
	if (!pFocused)
		return 0;

	TMVector2 vec = pFocused->m_vecPosition;

	int nMyX128 = (int)vec.x / 128;
	int nMyY128 = (int)vec.y / 128;

	if ((nMyX128 == 13 && nMyY128 == 31) || (nMyX128 == 14 && nMyY128 == 30)
		|| (nMyX128 == 15 && nMyY128 == 31) || (nMyX128 == 18 && nMyY128 == 30))
	{
...
{
	// Selchar carrega o mapa inteiro em Character.dat; a chave visual por
	// setor (feita para o campo) escondia o chão e boa parte do cenário.

...
namespace
{
int g_uiClickDiagLines = 0;

void UiClickDiag(int nX, int nY, const SControl* ctrl, int absX, int absY)
{
	if (g_uiClickDiagLines >= 300)
		return;
	++g_uiClickDiagLines;
	if (FILE* f = fopen("sdmc:/switch/client748/wyd748_diag.txt", "ab"))
	{
		if (ctrl)
			fprintf(f, "[UI] lbtn %d,%d -> id=%u type=%d rect=%d,%d %dx%d\n", nX, nY,
				ctrl->m_dwControlID, static_cast<int>(ctrl->m_eCtrlType), absX, absY,
				static_cast<int>(ctrl->m_nWidth), static_cast<int>(ctrl->m_nHeight));
		else
			fprintf(f, "[UI] lbtn %d,%d -> nenhum controle\n", nX, nY);
		fclose(f);
	}
}
}
...
#ifdef WYD_LINUX
#if defined(__SWITCH__)
	const int hideCursor = 0;
#else
	const int hideCursor = (g_bTouchUi && TouchControlsUI::Instance().IsFeatureEnabled());
#endif
	if (m_pCursor && m_pCursor->m_bVisible && !hideCursor)
		...
		if (!pCurrentControl->m_cDeleted && pCurrentControl->m_bVisible)
		{
			...
			if (ret == 1)
			{
				...
				if (dwFlags == WM_LBUTTONDOWN)
					UiClickDiag(nX, nY, pCurrentControl,
...
#if defined(__SWITCH__)
	if (dwFlags == WM_LBUTTONDOWN && !bProcessed)
		UiClickDiag(nX, nY, nullptr, 0, 0);
#endif
	if (!bProcessed)
	{
...
int SControlContainer::FrameMove(unsigned int dwServerTime)
{
	...
	do
	{
		if (!pCurrentControl->m_cDeleted)
		{
			...
			{
				// AlwaysOnTop controls (item desc panel) must be drawn once at the
				// end. GeomControl uses a single m_pNextGeom link: adding the same
				// geom twice in one frame corrupts layer lists and redraws the
				// opaque plate over its own text ("escuro").
				const int deferAlwaysOnTop = pCurrentControl->m_bAlwaysOnTop;
				if (!deferAlwaysOnTop)
					pCurrentControl->FrameMove2(m_pDrawControl, vParentPos, vControlLayer, 0);

				if (!deferAlwaysOnTop && pCurrentControl->m_pDown)
				{
	...
	// Item description (panel 258) is first in FieldScene2; paint plate then text
	// above the rest of the UI, under the cursor (layer 29).
	if (g_pCurrentScene != nullptr)
	{
		auto* pDesc = g_pCurrentScene->m_pDescPanel;
		if (pDesc && pDesc->IsVisible() && pDesc->m_bAlwaysOnTop)
		{
			TMVector2 origin{};
			pDesc->FrameMove2(m_pDrawControl, origin, 27, 0);
			// Depth-first: FieldScene2 may nest under 258; a flat NextLink walk
			// would skip nested text and leave only the gray plate.
			SControl* stack[64]{};
			TMVector2 stackParent[64]{};
			int sp = 0;
			if (pDesc->m_pDown && sp < 64)
			{
				stack[sp] = static_cast<SControl*>(pDesc->m_pDown);
				stackParent[sp] = TMVector2{ pDesc->m_nPosX, pDesc->m_nPosY };
				++sp;
			}
			while (sp > 0)
			{
				--sp;
				SControl* ch = stack[sp];
				const TMVector2 parentPos = stackParent[sp];
				if (ch->m_bVisible)
					ch->FrameMove2(m_pDrawControl, parentPos, 28, 0);
				if (ch->m_pNextLink && sp < 64)
				{
					stack[sp] = static_cast<SControl*>(ch->m_pNextLink);
					stackParent[sp] = parentPos;
					++sp;
				}
				if (ch->m_pDown && sp < 64)
				{
					stack[sp] = static_cast<SControl*>(ch->m_pDown);
					stackParent[sp] = TMVector2{
						parentPos.x + ch->m_nPosX,
						parentPos.y + ch->m_nPosY
					};
					++sp;
				}
			}
...
#ifdef WYD_LINUX
	if (g_bTouchUi)
		TouchControlsUI::Instance().FrameMove(dwServerTime);
#endif

	// Com Touch UI ativo, não desenha o cursor do mouse.
#ifdef WYD_LINUX
#if defined(__SWITCH__)
	const int hideCursor = 0;
#else

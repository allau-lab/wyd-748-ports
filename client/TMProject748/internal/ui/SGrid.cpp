...
int SGridControl::OnMouseEvent(unsigned int dwFlags, unsigned int wParam, int nX, int nY)
{
	...
	// --- Touch / 2 toques: equipar, desequipar, inventário↔cargo ---
	if (bPtInRect && g_pCursor && g_pCursor->GetStyle() == ECursorStyle::TMC_CURSOR_HAND
		&& (dwFlags == 515 || dwFlags == WM_LBUTTONDBLCLK))
	{
		if (SmartMoveItem(nCellX, nCellY))
			return 1;
	}
	if (dwFlags == 514 && bPtInRect && g_pCursor
		&& (g_pCursor->GetStyle() == ECursorStyle::TMC_CURSOR_HAND
			|| g_pCursor->GetStyle() == ECursorStyle::TMC_CURSOR_PICKUP))
	{
		// Switch: não filtrar por LastTouchWasTap — o jitter do toque zerava
		// allowDbl e impedia o 2º toque de desequipar. Sempre aceitar
		// mesmo-célula em <450 ms; no 1º UP com item anexado, sair cedo para
		// não cair no SwapItem da mesma célula (que cancelava o arraste→inv).
		static unsigned int s_dblGrid = 0;
		static int s_dblX = -1, s_dblY = -1;
		static unsigned int s_dblTime = 0;
		const unsigned int now = g_pTimerManager ? g_pTimerManager->GetServerTime() : 0;
		if (s_dblGrid == m_dwControlID && s_dblX == nCellX && s_dblY == nCellY
			&& now && s_dblTime && now - s_dblTime < 450)
		{
			s_dblTime = 0;
...
#if defined(__SWITCH__)
	// Arrastar: anexa no DOWN para o UP em outro slot completar o SwapItem.
	if (dwFlags == 513 && bPtInRect && g_pCursor
		&& g_pCursor->GetStyle() == ECursorStyle::TMC_CURSOR_HAND && !g_pCursor->m_pAttachedItem
		&& (m_eGridType == TMEGRIDTYPE::GRID_DEFAULT || m_eGridType == TMEGRIDTYPE::GRID_CARGO
			|| m_eItemType != TMEITEMTYPE::ITEMTYPE_NONE))
	{
		auto* pPick = SelectItem(nCellX, nCellY);
		if (pPick && pPick->m_GCObj.dwColor != 0xFFFF0000)
		{
			g_pCursor->AttachItem(pPick);
			SGridControl::m_pLastAttachedItem = pPick;
			return 1;
		}
	}
#endif
	...
	if (!bClick && dwFlags == 512)
	{
		...
		// Height/position must use the lines MouseOver just wrote; otherwise the
		// native 409px plate opens at the cursor and text sits off-screen.
		CommitVisibleDescPanelLayout();
		if (nRet != 2)
			return nRet;
	}
	else if (dwFlags == 516)
...
void SGridControl::FrameMove2(stGeomList* pDrawList, TMVector2 ivParentPos, int inParentLayer, int nFlag)
{
		...
		m_vecPickupedPos.y >= 0 && m_vecPickupedPos.y < m_nRowGridCount)
	{
		...
		const float cellWidth = m_nWidth / static_cast<float>(m_nColumnGridCount);
		const float cellHeight = m_nHeight / static_cast<float>(m_nRowGridCount);
		const int visibleWidth = std::min(m_vecPickupedSize.x, m_nColumnGridCount - m_vecPickupedPos.x);
		const int visibleHeight = std::min(m_vecPickupedSize.y, m_nRowGridCount - m_vecPickupedPos.y);
...
void SGridControl::UpdateDescPanelHeight()
{
	...
	float descPanelHeight = 0.0f;
	const unsigned int lastId = pFScene->GetLascDescParamId();
	auto* lastParamText = pFScene->m_pParamText[lastId];
	if (lastParamText && lastParamText->GetText()[0] != 0)
	{
		descPanelHeight = lastParamText->m_nPosY + lastParamText->m_Font.m_FontSize + 25.0f;
	}
	else if (pFScene->m_pDescNameText && pFScene->m_pDescNameText->GetText()[0] != 0)
	{
		descPanelHeight = pFScene->m_pDescNameText->m_nHeight + 12.0f;
	}
	else
		return;

	// FieldScene2 ships panel 258 at ~410px; never keep that empty gray slab.
	if (descPanelHeight < 40.0f)
		descPanelHeight = 40.0f;
	if (descPanelHeight > 280.0f)
		descPanelHeight = 280.0f;

	pFScene->m_pDescPanel->SetHeight(descPanelHeight);
}

void SGridControl::CommitVisibleDescPanelLayout()
{
	auto pFScene = static_cast<TMFieldScene*>(g_pCurrentScene);
	if (!pFScene || !pFScene->m_pDescPanel)
		return;
	...
	if (!pDescPanel->IsVisible())
		return;

	auto* pDescNameText = pFScene->m_pDescNameText;
	// Visible plate with no name is the "gray with no info" bug report.
	if (!pDescNameText || pDescNameText->GetText()[0] == 0)
	{
	...
	pDescNameText->SetVisible(1);
	for (int i = 0; i < NUM_ITEM_DESC_PARAMS; ++i)
	{
		if (pFScene->m_pParamText[i])
			pFScene->m_pParamText[i]->SetVisible(1);
	}

	const auto vecCursorPos = g_pCursor->GetPos();
	float nPosX = 0.0f;
	float nPosY = 0.0f;
...
void SGridControl::BuyItem(int nCellX, int nCellY)
{
	...
	{
		auto pItem = GetItem(nCellX, nCellY);
		if (pItem && pItem->m_pItem && g_pCurrentScene && g_pCurrentScene->m_pMessageBox)
		{
			char szMsg[128]{};
...
int SGridControl::MouseOver(int nCellX, int nCellY, int bPtInRect)
{
	...
	// Paint above inventory/equipment (FieldScene2 inserts panel 258 first).
	pDescPanel->SetAlwaysOnTop(1);
	pDescPanel->SetVisible(0);
...
char SGridControl::SmartMoveItem(int nCellX, int nCellY)
{
	auto* pFScene = static_cast<TMFieldScene*>(g_pCurrentScene);
	if (!pFScene || !pFScene->m_pMyHuman)
		return 0;
	if (m_eGridType != TMEGRIDTYPE::GRID_CARGO && m_eGridType != TMEGRIDTYPE::GRID_DEFAULT
		&& m_eItemType == TMEITEMTYPE::ITEMTYPE_NONE)
		return 0;

	// Guarda-carga aberto: inventário ↔ cargo (mesma lógica do Ctrl+clique).
	if (pFScene->m_pCargoPanel && pFScene->m_pCargoPanel->m_bVisible
		&& (m_eGridType == TMEGRIDTYPE::GRID_CARGO || m_eGridType == TMEGRIDTYPE::GRID_DEFAULT)
		&& m_eItemType == TMEITEMTYPE::ITEMTYPE_NONE)
	{
	...
	NewItemPos = BASE_GetItemAbility(pItem->m_pItem, 17);
	if (NewItemPos <= 1 && CheckPos(m_eItemType) == -1)
		return 0;

	const int sDestType = CheckType(pItem->m_pGridControl->m_eItemType, pItem->m_pGridControl->m_eGridType);
	int sDestPos = CheckPos(pItem->m_pGridControl->m_eItemType);
	const int nAX = pItem->m_nCellIndexX;
	const int nAY = pItem->m_nCellIndexY;
	auto* pMobData = &g_pObjectManager->m_stMobData;

	// Inventário / cargo → equipar no slot correto.
	if (sDestPos == -1)
	{
		if (!BASE_CanEquip(pItem->m_pItem, &pMobData->CurrentScore, sDestPos, pMobData->Equip[0].sIndex,
				pMobData->Equip, g_pObjectManager->m_stSelCharData.Equip[g_pObjectManager->m_cCharacterSlot][0].sIndex,
				pMobData->HasSoulSkill()))
			return 0;
		if (NewItemPos >= 64 && NewItemPos <= 192)
		{
			...
			itemL.sIndex = g_pCurrentScene->m_pMyHuman->m_sLeftIndex;
			itemR.sIndex = g_pCurrentScene->m_pMyHuman->m_sRightIndex;
			const unsigned int nWeaponLPos = BASE_GetItemAbility(&itemL, 17);
			const unsigned int nWeaponRPos = BASE_GetItemAbility(&itemR, 17);
			if (nWeaponLPos == 64 && nWeaponRPos == 128 && NewItemPos != 128)
				NewItemPosConv = 6;
			if (nWeaponLPos == 64 && NewItemPos == 192)
				NewItemPosConv = 6;
			...
			if (!BASE_CanEquip(pItem->m_pItem, &pMobData->CurrentScore, 6, pMobData->Equip[0].sIndex, pMobData->Equip,
					g_pObjectManager->m_stSelCharData.Equip[g_pObjectManager->m_cCharacterSlot][0].sIndex,
					pMobData->HasSoulSkill()) && NewItemPos != 128)
				return 0;
		}
		MSG_SwapItem stSwapItem{};
		stSwapItem.Header.ID = g_pObjectManager->m_dwCharID;
		stSwapItem.Header.Type = MSG_SwapItem_Opcode;
		stSwapItem.SourType = 0;
		stSwapItem.SourPos = static_cast<char>(NewItemPosConv);
		stSwapItem.DestType = static_cast<char>(sDestType);
		stSwapItem.TargetID = TMFieldScene::m_dwCargoID;
		if (pItem->m_pGridControl->m_eGridType == TMEGRIDTYPE::GRID_CARGO)
			stSwapItem.DestPos = static_cast<char>(pFScene->GetCargoSlotForCell(this, nAX, nAY));
		else if (sDestType)
			stSwapItem.DestPos = static_cast<char>(nAX + 9 * nAY);
		else
			stSwapItem.DestPos = static_cast<char>(sDestPos);
		if (stSwapItem.DestPos != stSwapItem.SourPos || stSwapItem.SourType != stSwapItem.DestType)
		{
			SendOneMessage(reinterpret_cast<char*>(&stSwapItem), 20);
			pFScene->m_pMouseOverHuman = nullptr;
		}
		return 1;
	}

	// Item equipado → inventário.
	auto* pMyGrid = pFScene->m_pGridInv;
	if (!pMyGrid)
		return 0;
	int nGridIndex = BASE_GetItemAbility(pItem->m_pItem, 33);
	if (nGridIndex > 7 || nGridIndex < 0)
		nGridIndex = 0;
	const IVector2 vecGrid = pMyGrid->CanAddItemInEmpty(g_pItemGridXY[nGridIndex][0], g_pItemGridXY[nGridIndex][1]);
	if (vecGrid.x == -1)
		return 0;
	MSG_SwapItem Msg{};
	Msg.Header.ID = g_pObjectManager->m_dwCharID;
	Msg.Header.Type = MSG_SwapItem_Opcode;
	Msg.SourType = 1;
	Msg.SourPos = static_cast<char>(vecGrid.x + 9 * vecGrid.y);
	Msg.DestType = static_cast<char>(sDestType);
	Msg.DestPos = static_cast<char>(sDestPos);
	Msg.TargetID = TMFieldScene::m_dwCargoID;
	if ((unsigned char)sDestPos != (unsigned char)Msg.SourPos || Msg.SourType != Msg.DestType)
	{
		SendOneMessage(reinterpret_cast<char*>(&Msg), 20);
		pFScene->m_pMouseOverHuman = nullptr;
	}
	return 1;
}
	...
	: S3DObj(0, inX, inY, 24.0f, 24.0f)
{
	...
	if (m_pItem && WYD748_IsValidItemIndex(pItem->sIndex))
	{
		...
		if (sMultiTexture > 12)
			sMultiTexture = 12;
		// Preserve the refinement texture used by the native UI renderer.
		if (itemDef.nIndexMesh >= 0)

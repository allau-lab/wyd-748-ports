...
{
	// Panels from the optional account manager use texture -2. On SDL3 a
	// missing atlas entry must not become a white full-screen modal; use the
	// original WYD dark glass fallback for the large panel.
	if (m_GCPanel.nTextureSetIndex < 0 &&
		(m_GCPanel.dwColor & 0x00FFFFFFu) == 0x00FFFFFFu &&
		(m_nWidth > 260.0f || m_nHeight > 260.0f))
		m_GCPanel.dwColor = 0xE61C1715u;
...
void SEditableText::Update()
{
	...
	{
		// Máscara ASCII '*': o literal '•' (U+2022) truncado em char vira 0xA2,
		// que o pipeline CP949 emparelha com o byte seguinte e rende glifos
		// aleatórios (×××) na fonte NanumGothic.
...
int SButton::OnMouseEvent(unsigned int dwFlags, unsigned int wParam, int nX, int nY)
{
	...
	case WM_LBUTTONUP:
		...
		if (m_bEnable == 1 && m_bOver == 1 && m_bPressed == 1 && m_pEventListener != nullptr)
		{
			...
			// Botões de + ponto (status/mastery): debounce curto para clique rápido.
			const unsigned int id = m_dwControlID;
			const bool scorePointBtn =
				id == 1073 || id == 1076 || id == 1079 || id == 1088 ||
				id == 1139 || id == 1142 || id == 1145 || id == 1154 ||
				id == 65716 || id == 65719 || id == 65722 || id == 65725 ||
				id == 65754 || id == 65757 || id == 65760 || id == 65763;
			const unsigned int clickCooldown = scorePointBtn ? 80u : 500u;
			if (dwServerTime > m_dwOldTime + clickCooldown)
			{
...
void SButton::FrameMove2(stGeomList* pDrawList, TMVector2 ivParentPos, int inParentLayer, int nFlag)
{
	...
	// SDL3/DXVK has no valid texture for the legacy "-2" button skin. The
	// renderer otherwise falls back to an opaque white rectangle, obscuring
	// the native dark/brown panel underneath (PIN2 and account manager).
	// Preserve hit testing and text, but make that fallback translucent.
	if (m_GCPanel.nTextureSetIndex < 0 &&
		(m_GCPanel.dwColor & 0x00FFFFFFu) == 0x00FFFFFFu)
		m_GCPanel.dwColor = 0x01010101u;

	// PIN2 is a later-client overlay whose button atlas is not distributed in
	// the 7.48 Linux asset set. Keep the original dark plates and render the
	// labels with the same white font as the native dialog.
	const bool linuxNativeDarkButton =
		(m_dwControlID >= 66433u && m_dwControlID <= 66446u) ||
		m_dwControlID == 4613u || m_dwControlID == 4616u || m_dwControlID == 5673u;
	if (linuxNativeDarkButton)
	{
		if (m_GCPanel.pFont == nullptr && m_pAltText != nullptr)
		{
			m_GCPanel.pFont = new TMFont2();
			strncpy(m_GCPanel.strString, m_pAltText->GetText(), sizeof(m_GCPanel.strString) - 1);
			m_GCPanel.strString[sizeof(m_GCPanel.strString) - 1] = 0;
			m_GCPanel.pFont->SetText(m_GCPanel.strString, 0xFFFFFFFFu, 0);
		}
		m_GCPanel.nTextureSetIndex = -2;
		m_GCPanel.dwColor = 0x01010101u;
	}
		...
		0x1010101, RENDERCTRLTYPE::RENDER_IMAGE_STRETCH)
{
	...
	// Alvos de toque maiores nos botões Sim/Não (pads de teleporte, etc.).
	const float minH = 32.0f * RenderDevice::m_fHeightRatio;
	const float minW = 110.0f * RenderDevice::m_fWidthRatio;
	if (m_pOKButton)
	{
		const float w = m_pOKButton->m_nWidth < minW ? minW : m_pOKButton->m_nWidth;
		const float h = m_pOKButton->m_nHeight < minH ? minH : m_pOKButton->m_nHeight;
		m_pOKButton->SetSize(w, h);
	}
	if (m_pCancelButton)
	{
		const float w = m_pCancelButton->m_nWidth < minW ? minW : m_pCancelButton->m_nWidth;
		const float h = m_pCancelButton->m_nHeight < minH ? minH : m_pCancelButton->m_nHeight;
		m_pCancelButton->SetSize(w, h);
	}
#endif
}
...
int AddRenderControlItem(stGeomList* pDrawList, GeomControl* pGeomControl, int nLayer)
{
	...
	if (pGeomControl)
	{
		...
		// Each geom may appear in only one layer list per frame.
		pGeomControl->m_pNextGeom = nullptr;
	}

...
public:
    ...
    // After MouseOver fills lines, shrink+place the tooltip so the default
    // FieldScene2 height (~410px) never leaves a blank gray plate over the UI.
    void CommitVisibleDescPanelLayout();
    ...
    // Touch/2-cliques: equipa, desequipa ou inventário↔cargo.
    char SmartMoveItem(int nCellX, int nCellY);

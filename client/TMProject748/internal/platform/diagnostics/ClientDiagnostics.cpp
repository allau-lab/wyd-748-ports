...
namespace
{
...
#if defined(__linux__)
    void LinuxCrashSignalHandler(int signalNumber)
    {
        char line[256]{};
        std::snprintf(line, sizeof(line),
            "Fatal signal=%d pid=%d\r\n", signalNumber, static_cast<int>(getpid()));
        AppendLine(line);

        // Async-signal-safe enough for triage: capture a short native backtrace
        // into the same client-debug.log used by the 7.48 compatibility path.
        void* frames[64]{};
        const int frameCount = backtrace(frames, 64);
        AppendLine("Native backtrace:\r\n");
        ...
        if (symbols)
        {
            ...
            {
                char frameLine[512]{};
                std::snprintf(frameLine, sizeof(frameLine), "  #%d %s\r\n", i,
                    symbols[i] ? symbols[i] : "?");
                AppendLine(frameLine);
            }
            free(symbols);
        }
    ...
    void InstallLinuxCrashHandlers()
    {
        struct sigaction action {};
        action.sa_handler = LinuxCrashSignalHandler;
    ...
    void InstallPlatformCrashHandlers()
    {
#if defined(__linux__)
        InstallLinuxCrashHandlers();
#endif
    }
} // namespace

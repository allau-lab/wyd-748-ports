#ifdef WYD_LINUX
// Port Linux: BSD sockets + stubs mínimos. Source Win32 original permanece intacta.
...
#include <climits>
#include <cstring>
#include "../../wire/MessageHeader.h"
extern char EncodeByte[4];
extern HWND hWndMain;
extern unsigned int CurrentTime;
extern unsigned int LastSendTime;
#ifndef sin_addr_s_addr
#define sin_addr_s_addr(addr) ((addr).s_addr)
#endif
#include <cerrno>
#include <cstdio>
static void NetDiag(const char* what, const char* host, int port)
{
	const int err = errno;
#if defined(__SWITCH__)
	if (std::FILE* f = ::fopen("sdmc:/switch/client748/wyd748_diag.txt", "ab"))
	{
		std::fprintf(f, "[NET] %s %s:%d errno=%d (%s)\n", what, host ? host : "?", port, err, std::strerror(err));
		std::fclose(f);
	}
#else
	std::fprintf(stderr, "[NET] %s %s:%d errno=%d (%s)\n", what, host ? host : "?", port, err, std::strerror(err));
#endif
}
#else
#include "pch.h"
...
#include "TMGlobal.h"
#include "TMLog.h"
#define sin_addr_s_addr(addr) ((addr).S_un.S_addr)
#endif
...
unsigned int CPSock::StartListen(HWND hWnd, int ip, int port, int WSA)
{
	...
	sin_addr_s_addr(local_sin.sin_addr) = ip;
...
unsigned int CPSock::ConnectServer(char* HostAddr, int Port, int ip, int WSA)
{
	...
	sin_addr_s_addr(InAddr.sin_addr) = inet_addr(HostAddr);
	...
	{
#ifdef WYD_LINUX
		NetDiag("socket FALHOU", HostAddr, Port);
#endif
			...
			bind(tSock, (const struct sockaddr*)&local_sin, 16) != -1))
	{
		...
		else if (connect(tSock, (const struct sockaddr*)&InAddr, 16) >= 0)
		{
			...
			{
#ifdef WYD_LINUX
				NetDiag("connect OK", HostAddr, Port);
#endif
				Sock = tSock;
			...
			{
#ifdef WYD_LINUX
				NetDiag("WSAAsyncSelect FALHOU", HostAddr, Port);
#endif
				closesocket(tSock);
				Sock = 0;
				return 0;
		...
		{
#ifdef WYD_LINUX
			NetDiag("connect FALHOU", HostAddr, Port);
#endif
			WSAGetLastError();
			closesocket(tSock);
			Sock = 0;
	...
	{
#ifdef WYD_LINUX
		NetDiag("bind FALHOU", HostAddr, Port);
#endif
...
char* CPSock::ReadMessage(int* ErrorCode, int* ErrorType)
{
	...
	*ErrorCode = 0;
	...
	if (!Init)
	{
		...
		unsigned int InitCode = 0;
		std::memcpy(&InitCode, &pRecvBuffer[nProcPosition], sizeof(InitCode));
	...
	std::memcpy(&Size, &pRecvBuffer[nProcPosition], sizeof(Size));
	unsigned char iKeyWord = static_cast<unsigned char>(pRecvBuffer[nProcPosition + 2]);
	unsigned char KeyWord = pKeyWord[iKeyWord * 2];
	unsigned char CheckSum = static_cast<unsigned char>(pRecvBuffer[nProcPosition + 3]);
	// Type/ID are not used for framing; only Size/KeyWord/CheckSum are.
...
PacketView CPSock::ReadPacketView(int* ErrorCode, int* ErrorType)
{
	...
	return { header.Type, message, static_cast<std::size_t>(header.Size) };
}
...
char* CPSock::ReadMessage2(int* ErrorCode, int* ErrorType)
{
	...
	*ErrorCode = 0;
	...
	else if ((unsigned int)(nRecvPosition - nProcPosition) >= 12)
	{
		...
		unsigned short size = 0;
		std::memcpy(&size, pMsg, sizeof(size));

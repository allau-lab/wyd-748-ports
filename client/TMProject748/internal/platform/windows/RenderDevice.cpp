...
#ifdef __linux__
		// Outdoor field must never clear to pure black (void look). Dungeon flag
		// or an uninitialized sky can leave ActualClearColor=0 on DXVK.
		if (g_pCurrentScene != nullptr &&
			(g_pCurrentScene->GetSceneType() == ESCENE_TYPE::ESCENE_FIELD ||
			 g_pCurrentScene->GetSceneType() == ESCENE_TYPE::ESCENE_SELCHAR) &&
			(RenderDevice::m_bDungeon == 0 || RenderDevice::m_bDungeon == 3 ||
			 RenderDevice::m_bDungeon == 4) &&
			(m_dwActualClearColor & 0x00FFFFFFu) == 0)
		{
			m_dwActualClearColor =
				(g_pCurrentScene->GetSceneType() == ESCENE_TYPE::ESCENE_SELCHAR)
					? 0xFF555544u
					: 0xFF335599u;
		}
...
int RenderDevice::Unlock(int bEnd)
{
	if (bEnd == 1 && m_pFont != nullptr)
	{
		...
		if ((fTime - fLastTime) > 2.0f)
		{
			...
			// O diagnóstico pode ser ativado também em builds Release por
			// DEBUG_STATS=1; não depender de _DEBUG para preencher o texto.
			snprintf(szString, sizeof(szString),
				"FPS: %4.1f | %.2f ms | Objects: %d | Effects: %d",
				m_fFPS, m_fFPS > 0.0f ? (1000.0f / m_fFPS) : 0.0f,
				g_objectnumber, g_effectnumber);
		}
...
HRESULT RenderDevice::RestoreDeviceObjects()
{
	...
	// Keep UI projection aspect valid (SetMatrixForUI). Win32 CD3DApplication
	// filled this from the swap chain; DXVK Native does not.
	m_d3dsdBackBuffer.Width = m_dwScreenWidth;
	m_d3dsdBackBuffer.Height = m_dwScreenHeight;
	...
	if (m_hbmBitmap == nullptr)
	{
		...
		// Escala a fonte com a resolução (UI base 800×600). Em 16:9 (ex.: 1280×720)
		// WidthRatio > HeightRatio; escalar só pela largura faz o glifo mais alto
		// que a linha do controle (HeightRatio) e o texto sobrepõe — painel Y, HELP, etc.
		{
			float uiScale = RenderDevice::m_fWidthRatio;
			if (RenderDevice::m_fHeightRatio > 0.0f && RenderDevice::m_fHeightRatio < uiScale)
				uiScale = RenderDevice::m_fHeightRatio;
			int scaled = static_cast<int>(RenderDevice::m_nFontSize * uiScale + 0.5f);
			if (scaled < RenderDevice::m_nFontSize)
				scaled = RenderDevice::m_nFontSize;
			if (scaled > 64)
				scaled = 64;
			if (scaled != RenderDevice::m_nFontSize)
			{
				DeleteObject(m_hFont);
				m_hFont = CreateFont(scaled, 0, 0, 0, g_nFontBold, 0, 0, 0, 1, 4, 0, 4, 2, g_szFontName);
				RenderDevice::m_nFontSize = scaled;
			}
		}
...
{
	// config.txt ships with [BRIGHT] 0. That builds a flat (all-zero) ramp;
	// DXVK rejects it and SDL/Wayland can still darken the window. Treat the
	// mid-point 50 as "leave hardware gamma alone".
	if (RenderDevice::m_nBright == 50)
		return;
	if (RenderDevice::m_nBright < 10)
		RenderDevice::m_nBright = 10;
	if (RenderDevice::m_nBright > 90)
		RenderDevice::m_nBright = 90;

	{
		...
		for (int i = 0; i < 256; i++)
		{
			...
			gamma.red[i] = static_cast<WORD>(nVal);
			gamma.green[i] = static_cast<WORD>(nVal);
			gamma.blue[i] = static_cast<WORD>(nVal);
		}
...
int RenderDevice::InitVertexShader()
{
	...
	// UBYTE4 (5): DXVK treats D3DCOLOR (4) as normalized floats for BLENDINDICES,
	// which collapses bone indices and explodes skinned meshes into giants.
	VertexDecl1[1].Type = 5;
#else
...
#if defined(WYD_LINUX) || defined(__linux__)
			// D3DCOLOR BLENDINDICES expand to (R,G,B,A); shader uses .zyxw to put
			// memory byte0 (B) back in .x before *765. UBYTE4 maps byte0→.x already,
			// so .zyxw would scramble indices (detached limbs). Force .xyzw.
			{
				DWORD* dwords = static_cast<DWORD*>(pCode->GetBufferPointer());
				const UINT nDwords = pCode->GetBufferSize() / sizeof(DWORD);
				for (UINT di = 0; di < nDwords; ++di)
				{
					if (dwords[di] == 0x90c60002u) // v2.zyxw
						dwords[di] = 0x90e40002u; // v2.xyzw
				}
			}
#endif
...
int RenderDevice::SetRenderStateBlock(int nIndex)
{
	...
	{
#ifdef __linux__
		// DXVK Native: LightVal noturno da selchar (~0.26) e fog preto deixam
		// personagens com membros "invisíveis" (pretos no void) e o campo escuro.
		auto clampChan = [](float v, float lo) { return v < lo ? lo : v; };
		m_colorLight.r = clampChan(m_colorLight.r, 0.85f);
		m_colorLight.g = clampChan(m_colorLight.g, 0.85f);
		m_colorLight.b = clampChan(m_colorLight.b, 0.85f);
		m_colorLight.a = 1.0f;
		m_colorBackLight.r = clampChan(m_colorBackLight.r, 0.55f);
		m_colorBackLight.g = clampChan(m_colorBackLight.g, 0.55f);
		m_colorBackLight.b = clampChan(m_colorBackLight.b, 0.55f);
		m_colorBackLight.a = 1.0f;
#endif
		...
		{
#if defined(WYD_LINUX) || defined(__linux__)
			// 0xDD discards soft-alpha texels (faces, cloth AA) on DXVK DXT →
			// hollow torsos / missing limbs. Match the soft UI block threshold.
		...
		// Ambient 0x33 (~20%) + luz direcional fraca no DXVK Native deixa selchar
		// e campo quase pretos. O valor do InitializeScene era sobrescrito aqui
		// a cada frame — reforçar em ambas as cenas 3D do client 7.48.
		{
			DWORD ambient = 0x33FFFFFFu;
			if (g_pCurrentScene != nullptr)
			{
				const auto scene = g_pCurrentScene->GetSceneType();
				if (scene == ESCENE_TYPE::ESCENE_SELCHAR)
					ambient = 0xFFCCCCCCu;
				else if (scene == ESCENE_TYPE::ESCENE_FIELD)
					ambient = 0xFF999999u;
			}
...
#ifdef __linux__
		// Fog preto (m_dwClearColor herdado) engole o cenário no DXVK; desligar
		// até a cor de fog acompanhar o céu de forma estável.
		SetRenderState(D3DRS_FOGENABLE, 0);
#else
		...
		// SelectServer leaves m_dwClearColor=0. If the sky mesh/FrameMove has not
		// yet written the daytime fog RGB, FOGCOLOR stays black and the whole
		// field washes out (matches the "black void" screenshot on DXVK).
		DWORD fogClear = m_dwClearColor;
		if ((fogClear & 0x00FFFFFFu) == 0 &&
			g_pCurrentScene != nullptr &&
			g_pCurrentScene->GetSceneType() == ESCENE_TYPE::ESCENE_FIELD)
		{
			// TMSky daytime RGB (m_dwR/G/B[0] = 28,120,189)
			fogClear = 0x001C78BDu;
			m_dwClearColor = fogClear;
		}
		m_dwActualClearColor = fogClear;
		...
		SetRenderState(D3DRS_FOGCOLOR, fogClear);
...
#if defined(WYD_LINUX) || defined(__linux__)
		// O padrão melhora a nitidez do terreno, mas não deve ser imposto a
		// GPUs integradas/ARM64. O valor vem de ANISOTROPIC no config.txt.
		const DWORD aniso = static_cast<DWORD>(std::max(0, std::min(g_nAnisotropic, 16)));
		if (aniso > 1)
		{
			SetSamplerState(0, D3DSAMP_MAXANISOTROPY, aniso);
			SetSamplerState(0, D3DSAMP_MINFILTER, 3u); // D3DTEXF_ANISOTROPIC
			SetSamplerState(1u, D3DSAMP_MAXANISOTROPY, aniso);
			SetSamplerState(1u, D3DSAMP_MINFILTER, 3u);
		}
		SetSamplerState(1u, D3DSAMP_MIPFILTER, 2u);
#endif
		SetTextureStageState(0, D3DTSS_TEXCOORDINDEX, 0);
...
HRESULT RenderDevice::SetProjectionMatrix()
{
	D3DXMatrixPerspectiveFovLH(&m_matProj, 
		...
		g_ClipNear * 1.4f,
#if defined(WYD_LINUX) || defined(__linux__)
		// Never let a stale 70-unit ClipFar from TMSky shrink the projection.
		(g_ClipFar < 200.0f) ? 320.0f : g_ClipFar
#else
		g_ClipFar
#endif
		);
...
int RenderDevice::SetMatrixForUI()
{
	...
	// Prefer live viewport / screen size. m_d3dsdBackBuffer is never filled on the
	// Linux DXVK path (CD3DApplication used to own it on Win32), so Width/Height
	// stay 0 → aspect NaN → 3D HUD orbs (HP/MP) explode across the status bar.
	UINT bbW = m_d3dsdBackBuffer.Width;
	UINT bbH = m_d3dsdBackBuffer.Height;
	if (bbW == 0 || bbH == 0)
	{
		bbW = m_viewport.Width ? m_viewport.Width : m_dwScreenWidth;
		bbH = m_viewport.Height ? m_viewport.Height : m_dwScreenHeight;
	}
	if (bbW == 0)
		bbW = 800;
	if (bbH == 0)
		bbH = 600;
	const float aspect = (float)bbW / (float)bbH;
	...
	D3DXMatrixPerspectiveFovLH(&matUIProjection, 0.1f, aspect, 10.0f, 100.0f);
...
void RenderDevice::RenderRectNoTex(float iX, float iY, float iCX, float iCY, DWORD dwColor, int bTrans)
{
	...
	for (int j = 0; j < 4; ++j)
	{
		m_CtrlVertex[j].position.z = 0.0f;
		m_CtrlVertex[j].rhw = 1.0f;
		m_CtrlVertex[j].diffuse = dwColor;
		m_CtrlVertex[j].tu = 0.0f;
		m_CtrlVertex[j].tv = 0.0f;
	}
...
{
#if defined(WYD_LINUX) || defined(__linux__)
	// Ícones touch custom (touch_icons/*.tga) — ver TouchControlsUI.
	if (ipControl->nTextureSetIndex == -9999)
	{
		extern void WYD_Touch_RenderCustomIcon(RenderDevice* rd, GeomControl* c);
		WYD_Touch_RenderCustomIcon(this, ipControl);
		return;
	}
#endif

...
int TMFont3::FrameMove(unsigned int dwServerTime)
{
	...
	if (m_bViewHalf)
	{
		// Metade do lifetime: aparece já no início do intervalo útil.
		m_bVisible = fProgress < 0.69999999f;
		m_fVisualProgress = fProgress / 0.69999999f;
	...
	{
		// Número de dano em tempo real: sem esperar 30% do lifetime (~450 ms).
		m_bVisible = 1;
		m_fVisualProgress = fProgress;
		if (m_fVisualProgress >= 0.80000001f)
			m_fVisualProgress = (float)(m_fVisualProgress - 0.80000001f) / 0.2f;
		else
			m_fVisualProgress = 0.0f;
